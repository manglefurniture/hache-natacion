...
function hache_sharky_whatsapp_apply_natural_venue_preference(array $state,string $text): array
{
    ...
    if(is_array($flow)&&($flow['name']??'')==='qualify_prospect'&&($flow['step']??'')==='sede'&&($flow['data']['venue_proposal']??null)==='MONTEVERDE'&&hache_sharky_whatsapp_monteverde_rejection($text))return $state;
...
function hache_sharky_whatsapp_qualification_input(PDO $pdo,array $state,array $event,int $now,int $minAge=12): ?array
{
    ...
    if(in_array($step,['swim','swim_conflict'],true)){
...
function hache_sharky_whatsapp_process(PDO $pdo,array $event,callable $conversationAnswer,array $extraContext=[]): array
{
    ...
    try{
        ...
        $levelConflict=hache_sharky_whatsapp_swim_level_conflict($state,$event,(int)($context['now']??time()));
