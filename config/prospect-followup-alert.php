...
function hache_internal_prospect_followup_durable_pause_evidence(PDO $pdo,array $hashes): array
{
    ...
    foreach($groups as $hash=>$rows){
        ...
        if(!$known||!is_array($bestEvent)){$out[$hash]=['known'=>false,'paused'=>false,'received_at'=>''];continue;}
        $out[$hash]=['known'=>true,'paused'=>hache_internal_prospect_followup_event_paused($bestEvent),'received_at'=>$receivedAt];
    }
    return $out;
}

function hache_internal_prospect_followup_due(array $row,array $state,?DateTimeImmutable $now=null,int $thresholdSeconds=HACHE_INTERNAL_PROSPECT_FOLLOWUP_THRESHOLD_SECONDS): bool
{
    ...
    try{$lastAt=new DateTimeImmutable($last,new DateTimeZone('America/Cancun'));}catch(Throwable $e){return false;}
    $now=hache_instante_operativo($now);
    $elapsed=$now->getTimestamp()-$lastAt->getTimestamp();
    return $elapsed>=max(3600,$thresholdSeconds);
}

function hache_internal_prospect_followup_candidates(PDO $pdo,?DateTimeImmutable $now=null,?array $settings=null): array
{
    ...
    $thresholdSeconds=max(1,(int)($settings['prospect_followup_hours']??24))*3600;
    ...
    $now=hache_instante_operativo($now);$out=[];
    ...
    foreach($contacts as $contact){
        ...
        $last=hache_sharky_crm_last_contact((string)($inbound[$hash]??''),(string)($outbound[$hash]??''),(string)($contact['last_seen_at']??''));
        $management=$latest[$hash]??null;
        $managementState=hache_sharky_crm_management_state(is_array($management)?$management:null,isset($last['at'])?(string)$last['at']:null,(int)($inboundCounts[$hash]??0),(int)($outboundCounts[$hash]??0));
        $ruleRow=[
            'gestion_disponible'=>true,'gestion_estado'=>$managementState,'ultimo_contacto'=>$last['at']??null,
            'pausa_durable_disponible'=>$state?true:(($pauseEvidence['known']??false)===true),
            'seguimiento_pausado'=>(($pauseEvidence['paused']??false)===true),
        ];
        if(!hache_internal_prospect_followup_due($ruleRow,$state,$now,$thresholdSeconds))continue;
        ...
        $out[]=[
            'contact_hash'=>$hash,'ultimo_contacto'=>(string)$last['at'],'gestion_estado'=>$managementState,'sede'=>$sede,
            'horas_sin_seguimiento'=>(int)floor(max(0,$now->getTimestamp()-$lastAt->getTimestamp())/3600),
            'umbral_horas'=>(int)$settings['prospect_followup_hours'],
        ];
    }
    return $out;
}
...
function hache_internal_prospect_followup_summary(array $candidates): array
{
    ...
    foreach($candidates as $candidate){$sede=(string)($candidate['sede']??'');if(isset($porSede[$sede]))$porSede[$sede]++;else $porSede['SIN_SEDE']++;}
