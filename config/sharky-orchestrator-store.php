...
function hache_sharky_orchestrator_contact_key(): string
{
    $key=hache_sharky_orchestrator_secret('SHARKY_CONTACT_HASH_KEY');
    if(strlen($key)>=32) return $key;
    if(PHP_SAPI==='cli') return 'hache-sharky-cli-regression-key-2026';
    throw new RuntimeException('SHARKY_CONTACT_HASH_KEY is required before enabling Sharky 2.0');
}

function hache_sharky_orchestrator_contact_hash(string $contact): string
{
    $digits=preg_replace('/\D+/','',$contact)?:'';
    if($digits==='') throw new InvalidArgumentException('Invalid empty contact');
    return hash_hmac('sha256','hache-sharky-contact-v3|'.$digits,hache_sharky_orchestrator_contact_key());
}
...
function hache_sharky_orchestrator_store_ready(PDO $pdo): bool
{
    ...
    try{
        foreach(['sharky_message_receipts','sharky_referrals','sharky_action_audit','sharky_outbox'] as $table){
    ...
    }catch(Throwable $e){return $memo[$key]=false;}
}
...
function hache_sharky_orchestrator_claim_message(PDO $pdo,string $messageId,string $contactHash,string $type): bool
{
    ...
    try{
        $st=$pdo->prepare('INSERT IGNORE INTO sharky_message_receipts(message_id,contact_hash,message_type,lease_until,attempt_count) VALUES(:m,:c,:t,DATE_ADD(NOW(),INTERVAL '.HACHE_SHARKY_MESSAGE_LEASE_SECONDS.' SECOND),1)');
        $st->execute([':m'=>$messageId,':c'=>$contactHash,':t'=>mb_substr($type,0,30)]);
        if($st->rowCount()===1) return true;
        $st=$pdo->prepare('UPDATE sharky_message_receipts SET contact_hash=:c,message_type=:t,lease_until=DATE_ADD(NOW(),INTERVAL '.HACHE_SHARKY_MESSAGE_LEASE_SECONDS.' SECOND),attempt_count=attempt_count+1 WHERE message_id=:m AND processed_at IS NULL AND (lease_until IS NULL OR lease_until<NOW())');
        $st->execute([':m'=>$messageId,':c'=>$contactHash,':t'=>mb_substr($type,0,30)]);
        return $st->rowCount()===1;
    }catch(Throwable $e){error_log('[sharky-orchestrator] durable message claim failed');return false;}
}

function hache_sharky_orchestrator_mark_processed(PDO $pdo,string $messageId): bool
{
    if(!hache_sharky_orchestrator_store_ready($pdo)) return false;
    $messageId=mb_substr(trim($messageId),0,191);if($messageId==='')return true;
    if(function_exists('hache_sharky_action_delivery_pending_for_message')&&hache_sharky_action_delivery_pending_for_message($pdo,$messageId))return false;
    try{
        ...
        return true;
    }catch(Throwable $e){error_log('[sharky-orchestrator] message processed mark failed');return false;}
}
...
{
    if(!hache_sharky_orchestrator_store_ready($pdo)) return false;
    $messageId=trim($messageId);if($messageId==='') return false;
    try{
        $referralJson=json_encode($referral,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
        $sql='INSERT IGNORE INTO sharky_referrals(message_id,contact_hash,alumno_id,source_type,source_id,ctwa_clid,source_url,headline,body,media_type,image_url,video_url,thumbnail_url,referral_json,captured_at) VALUES(:m,:c,:a,:st,:si,:clid,:url,:h,:b,:mt,:img,:vid,:thumb,:json,NOW())';
        $st=$pdo->prepare($sql);
        $st->execute([
        ...
        return $st->rowCount()===1;
    }catch(Throwable $e){error_log('[sharky-orchestrator] referral persistence failed');return false;}
}
...
function hache_sharky_orchestrator_action_begin(PDO $pdo,string $idempotencyKey,string $actionType,string $contactHash,?string $studentId,array $payload): bool
{
    ...
    try{
        ...
        return $st->rowCount()===1;
    }catch(Throwable $e){error_log('[sharky-orchestrator] action claim failed');return false;}
}
...
function hache_sharky_orchestrator_action_finish(PDO $pdo,string $idempotencyKey,bool $ok,string $resultCode): void
{
    ...
    }catch(Throwable $e){error_log('[sharky-orchestrator] action finish failed');}
}
...
{
    $dir=hache_sharky_orchestrator_runtime_dir('state');return $dir===''?'':$dir.'/'.hache_sharky_orchestrator_contact_hash($contact).'.json';
}
...
function hache_sharky_orchestrator_state_load(string $contact): array
{
    ...
    $raw=@file_get_contents($path);$state=is_string($raw)?json_decode($raw,true):null;
    if(!is_array($state)||(int)($state['updated_at']??0)<time()-86400){@unlink($path);return hache_sharky_orchestrator_state();}
...
{
    $path=hache_sharky_orchestrator_state_path($contact);if($path==='') return false;
    $json=json_encode($state,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);if($json===false) return false;
