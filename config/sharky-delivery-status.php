...
function hache_sharky_delivery_store_event(PDO $pdo,array $event): bool
{
    ...
    try{
        ...
        $st=$pdo->prepare($sql);
        $st->execute([
            ':id'=>(string)$event['provider_message_id'],
            ':s'=>(string)$event['status'],
            ':r'=>(int)$event['status_rank'],
            ':at'=>(string)$event['provider_event_at_utc'],
        ]);
        return true;
    }catch(Throwable $e){
        error_log('[sharky-delivery] status persistence failed');
        return false;
    }
}

/** @return array{seen:int,stored:int,schema_ready:bool} */
function hache_sharky_delivery_store_payload(PDO $pdo,array $payload,string $configuredPhoneId=''): array
{
    $events=hache_sharky_delivery_extract($payload);$seen=count($events);$stored=0;
    $ready=hache_sharky_delivery_schema_ready($pdo);
    if(!$ready)return ['seen'=>$seen,'stored'=>0,'schema_ready'=>false];
    foreach($events as $event){
        $phoneId=(string)$event['phone_number_id'];
        if($configuredPhoneId!==''&&$phoneId!==''&&!hash_equals($configuredPhoneId,$phoneId))continue;
        if(hache_sharky_delivery_store_event($pdo,$event))$stored++;
    }
    return ['seen'=>$seen,'stored'=>$stored,'schema_ready'=>true];
}
...
function hache_sharky_delivery_provider_message_id(string $response): string
{
    ...
    if(!is_array($messages)||!is_array($messages[0]??null))return '';
...
function hache_sharky_delivery_correlated_summary(PDO $pdo): array
{
    ...
    try{
        ...
        foreach($st->fetchAll(PDO::FETCH_ASSOC) as $row){
            $status=strtoupper((string)($row['status']??''));if(isset($counts[$status]))$counts[$status]=max(0,(int)($row['total']??0));
        }
        ...
        return [
            'correlated_total'=>array_sum($counts),
            'status_counts'=>$counts,
            'latest_provider_event_at_utc'=>$latest===false||$latest===null||$latest===''?null:(string)$latest,
        ];
    }catch(Throwable $e){
        return ['correlated_total'=>0,'status_counts'=>$counts,'latest_provider_event_at_utc'=>null];
    }
}
