...
/**
 * On a generic payment query this only reconciles already-created intents and
 * returns null so member-ops can render the fresh backend state. `member:pay`
 * owns the turn and returns a durable Mercado Pago checkout link.
 */
function hache_sharky_member_payment_process_event(PDO $pdo,array $event,array $business): ?bool
{
    ...
    if($intent==='payments')hache_sharky_member_payment_reconcile_student($pdo,$studentId);
    if($id!=='member:pay')return null;
    $ctx=hache_sharky_member_student_context($pdo,$contact);$pending=hache_sharky_member_payment_pending_from_context($ctx);
    ...
    try{
        ...
        if(!$pending){$payload=hache_sharky_whatsapp_text_payload($contact,'Ya no encuentro un saldo pendiente en tu inscripción activa. ✅');return hache_sharky_lab_queue_and_complete($pdo,$contact,$payload,(string)$event['id'].'|member-payment-clear',(string)$event['id']);}
