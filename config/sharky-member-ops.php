...
{
    $st=$pdo->prepare("SELECT se.id session_id,se.fecha,se.estado,se.motivo_cancelacion,se.cerrada,h.id horario_id,h.hora_inicio,h.hora_fin,s.clave sede_clave,s.nombre sede_nombre FROM profesor_horarios ph JOIN horarios h ON h.id=ph.horario_id JOIN sedes s ON s.id=h.sede_id JOIN sesiones se ON se.horario_id=h.id WHERE ph.profesor_id=:p AND ph.activo=1 AND h.activo=1 AND se.fecha BETWEEN :d AND :h ORDER BY se.fecha,h.hora_inicio LIMIT 20");
...
function hache_sharky_member_cancel_teacher_session(PDO $pdo,string $teacherId,string $sessionId,string $reason,string $actionKey): array
{
    ...
    try{
        ...
        if((int)($session['cerrada']??0)===1)throw new HacheSharkyBusinessException('Esa clase ya fue cerrada y no puede cancelarse desde Sharky.','SESSION_ALREADY_CLOSED',409);
