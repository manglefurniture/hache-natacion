...
{
    $payload['_sharky_allow_takeover']=true;return $payload;
}

function hache_sharky_outbox_add_venue_hints(array $payload): array
{
    if(($payload['type']??'')!=='interactive')return $payload;$interactive=$payload['interactive']??null;
    if(!is_array($interactive)||($interactive['type']??'')!=='button')return $payload;$buttons=$interactive['action']['buttons']??null;if(!is_array($buttons))return $payload;
    $hasMonteverde=false;$hasPalapas=false;
    foreach($buttons as $button){if(!is_array($button))continue;$id=strtolower(trim((string)($button['reply']['id']??'')));if($id==='sede:monteverde')$hasMonteverde=true;elseif($id==='sede:palapas')$hasPalapas=true;}
    if(!$hasMonteverde||!$hasPalapas)return $payload;
    foreach($buttons as $index=>$button){if(!is_array($button))continue;$id=strtolower(trim((string)($button['reply']['id']??'')));if($id==='sede:monteverde')$payload['interactive']['action']['buttons'][$index]['reply']['title']='Colegio Monteverde';elseif($id==='sede:palapas')$payload['interactive']['action']['buttons'][$index]['reply']['title']='Palapas Protudec';}
    $body=trim((string)($payload['interactive']['body']['text']??''));$monteverdeHint='📍 Colegio Monteverde — al inicio de Av. Bonampak';$palapasHint='📍 Palapas Protudec — a 100 m del Parque de las Palapas';
    if(!str_contains($body,'Av. Bonampak')&&!str_contains($body,'Parque de las Palapas')){$decorated=trim($body)."\n\n".$monteverdeHint."\n".$palapasHint;if(mb_strlen($decorated)<=1024)$payload['interactive']['body']['text']=$decorated;}
    return $payload;
}
...
function hache_sharky_outbox_add_sales_close(array $payload): array
{
    if(($payload['type']??'')!=='interactive')return $payload;$interactive=$payload['interactive']??null;
    if(!is_array($interactive)||($interactive['type']??'')!=='button')return $payload;$buttons=$interactive['action']['buttons']??null;if(!is_array($buttons))return $payload;
    $ids=[];foreach($buttons as $button){if(!is_array($button))continue;$id=strtolower(trim((string)($button['reply']['id']??'')));if($id!=='')$ids[]=$id;}
    if(!in_array('flow:yes',$ids,true)||!in_array('flow:no',$ids,true)||!in_array('flow:cancel',$ids,true))return $payload;
    $body=trim((string)($payload['interactive']['body']['text']??''));$normalized=mb_strtolower($body,'UTF-8');
    $isRegistrationOffer=str_contains($normalized,'registrarte al curso intensivo')||str_contains($normalized,'registrarte al intensivo')||str_contains($normalized,'continuemos con el intensivo');if(!$isRegistrationOffer)return $payload;
    $venue='';if(str_contains($normalized,'palapas'))$venue='Palapas Protudec';elseif(str_contains($normalized,'monteverde'))$venue='Colegio Monteverde';
    if($venue!=='')$message='Perfecto 😊 Ya tengo tu opción: curso intensivo en '.$venue.'.'."\n\n".'Si quieres, puedo ayudarte a apartar tu lugar ahora mismo. Elegimos fecha y horario y completamos tu inscripción.'."\n\n".'¿Cómo quieres continuar?';
    else $message='Perfecto 😊 Si quieres, puedo ayudarte a apartar tu lugar en el curso intensivo ahora mismo.'."\n\n".'Elegimos sede, fecha y horario y completamos tu inscripción.'."\n\n".'¿Cómo quieres continuar?';
    if(mb_strlen($message)>1024)return $payload;$payload['interactive']['body']['text']=$message;
    $payload['interactive']['action']['buttons']=[['type'=>'reply','reply'=>['id'=>'flow:yes','title'=>'Apartar mi lugar']],['type'=>'reply','reply'=>['id'=>'action:human','title'=>'Hablar con un profe']],['type'=>'reply','reply'=>['id'=>'flow:pause','title'=>'Ahora no']]];return $payload;
}
...
{
    if(is_array($payload['_sharky_sequence']??null))return hache_sharky_outbox_enqueue_sequence($pdo,$contact,$payload['_sharky_sequence'],$dedupeSeed);
    if(!is_array($payload['_sharky_payment_reminder']??null))$payload=hache_sharky_payment_reminder_prepare_registration_outbound($pdo,$contact,$payload,$dedupeSeed);
    if(!is_array($payload['_sharky_followup']??null)&&!is_array($payload['_sharky_payment_reminder_arm']??null))$payload=hache_sharky_followup_prepare_normal_outbound($pdo,$contact,$payload,$dedupeSeed);
...
function hache_sharky_outbox_owner_token(): string{return bin2hex(random_bytes(24));}

function hache_sharky_outbox_claim(PDO $pdo,int $limit=10,string $contactHash=''): array
{
    $limit=max(1,min(50,$limit));$rows=[];$contactHash=strtolower(trim($contactHash));if($contactHash!==''&&!preg_match('/^[a-f0-9]{64}$/',$contactHash))return [];
    try{
        if($contactHash!==''){$select=$pdo->prepare("SELECT id FROM sharky_outbox WHERE contact_hash=:c AND status='PENDING' AND available_at<=NOW() AND (lease_until IS NULL OR lease_until<NOW()) ORDER BY created_at,id LIMIT ".$limit);$select->execute([':c'=>$contactHash]);$ids=$select->fetchAll(PDO::FETCH_COLUMN);}
        else $ids=$pdo->query("SELECT id FROM sharky_outbox WHERE status='PENDING' AND available_at<=NOW() AND (lease_until IS NULL OR lease_until<NOW()) ORDER BY created_at,id LIMIT ".$limit)->fetchAll(PDO::FETCH_COLUMN);
        ...
        foreach($ids as $id){$owner=hache_sharky_outbox_owner_token();$claim->execute([':id'=>$id,':o'=>$owner]);if($claim->rowCount()!==1)continue;$read->execute([':id'=>$id,':o'=>$owner]);$row=$read->fetch(PDO::FETCH_ASSOC);if($row)$rows[]=$row;}
    }catch(Throwable $e){error_log('[sharky-outbox] claim failed');}return $rows;
}

function hache_sharky_outbox_renew_owner(PDO $pdo,string $id,string $ownerToken): bool
{
    if($id===''||$ownerToken==='')return false;try{$params=[':id'=>$id,':o'=>$ownerToken];$st=$pdo->prepare('UPDATE sharky_outbox SET lease_until=DATE_ADD(NOW(),INTERVAL '.HACHE_SHARKY_OUTBOX_LEASE_SECONDS.' SECOND) WHERE id=:id AND status=\'PENDING\' AND owner_token=:o');$st->execute($params);if($st->rowCount()===1)return true;$check=$pdo->prepare("SELECT 1 FROM sharky_outbox WHERE id=:id AND status='PENDING' AND owner_token=:o AND lease_until>=NOW() LIMIT 1");$check->execute($params);return (bool)$check->fetchColumn();}catch(Throwable $e){return false;}
}

function hache_sharky_outbox_release_owner(PDO $pdo,string $id,string $ownerToken): bool
{
    if($id===''||$ownerToken==='')return false;try{$st=$pdo->prepare("UPDATE sharky_outbox SET lease_until=NULL,owner_token=NULL WHERE id=:id AND status='PENDING' AND owner_token=:o");$st->execute([':id'=>$id,':o'=>$ownerToken]);return $st->rowCount()===1;}catch(Throwable $e){return false;}
}
...
{
    if($id===''||$ownerToken===''||$availableAt<=0)return false;try{$st=$pdo->prepare("UPDATE sharky_outbox SET available_at=FROM_UNIXTIME(:a),lease_until=NULL,owner_token=NULL,last_error=:e WHERE id=:id AND status='PENDING' AND owner_token=:o");$st->execute([':a'=>$availableAt,':e'=>mb_substr($reason,0,255),':id'=>$id,':o'=>$ownerToken]);return $st->rowCount()===1;}catch(Throwable $e){return false;}
}

function hache_sharky_outbox_mark_sent(PDO $pdo,string $id,string $ownerToken,string $providerMessageId=''): bool
{
    try{$providerMessageId=trim($providerMessageId);if($providerMessageId!==''&&strlen($providerMessageId)<=191&&hache_sharky_delivery_schema_ready($pdo)){$st=$pdo->prepare("UPDATE sharky_outbox SET status='SENT',sent_at=NOW(),provider_message_id=:pm,lease_until=NULL,owner_token=NULL,last_error=NULL WHERE id=:id AND status='PENDING' AND owner_token=:o");$st->execute([':pm'=>$providerMessageId,':id'=>$id,':o'=>$ownerToken]);return $st->rowCount()===1;}$st=$pdo->prepare("UPDATE sharky_outbox SET status='SENT',sent_at=NOW(),lease_until=NULL,owner_token=NULL,last_error=NULL WHERE id=:id AND status='PENDING' AND owner_token=:o");$st->execute([':id'=>$id,':o'=>$ownerToken]);return $st->rowCount()===1;}catch(Throwable $e){return false;}
}

function hache_sharky_outbox_mark_cancelled(PDO $pdo,string $id,string $ownerToken,string $reason='HUMAN_TAKEOVER'): bool
{
    try{$st=$pdo->prepare("UPDATE sharky_outbox SET status='CANCELLED',lease_until=NULL,owner_token=NULL,last_error=:e WHERE id=:id AND status='PENDING' AND owner_token=:o");$st->execute([':e'=>mb_substr($reason,0,255),':id'=>$id,':o'=>$ownerToken]);return $st->rowCount()===1;}catch(Throwable $e){return false;}
}

function hache_sharky_outbox_mark_failed(PDO $pdo,string $id,string $ownerToken,int $attempts,string $error='SEND_FAILED'): bool
{
    $next=$attempts+1;$status=$next>=8?'DEAD':'PENDING';$delay=min(3600,30*(2**min(6,$next-1)));try{$st=$pdo->prepare('UPDATE sharky_outbox SET status=:s,attempt_count=:a,last_error=:e,lease_until=NULL,owner_token=NULL,available_at=DATE_ADD(NOW(),INTERVAL '.$delay.' SECOND) WHERE id=:id AND status=\'PENDING\' AND owner_token=:o');$st->execute([':s'=>$status,':a'=>$next,':e'=>mb_substr($error,0,255),':id'=>$id,':o'=>$ownerToken]);return $st->rowCount()===1;}catch(Throwable $e){return false;}
}
...
function hache_sharky_outbox_dispatch(PDO $pdo,callable $sender,int $limit=10,string $lockedContact=''): array
{
    ...
    $stats=['sent'=>0,'failed'=>0,'dead'=>0,'cancelled'=>0];$limit=max(1,min(50,$limit));$lockedContact=preg_replace('/\D+/','',$lockedContact)?:'';if(hache_sharky_orchestrator_secret('SHARKY_ORCHESTRATOR_LAB_ENABLED')!=='1')return $stats;$lockedHash=$lockedContact!==''?hache_sharky_orchestrator_contact_hash($lockedContact):'';
    for($i=0;$i<$limit;$i++){
        ...
        $notAfter=(int)($payload['_sharky_not_after']??0);unset($payload['_sharky_not_after']);
        if($notAfter>0&&time()>=$notAfter){if(hache_sharky_outbox_mark_cancelled($pdo,$id,$owner,'NOT_AFTER_EXPIRED'))$stats['cancelled']++;continue;}
        ...
        try{
            ...
            if(hache_sharky_orchestrator_secret('SHARKY_ORCHESTRATOR_LAB_ENABLED')!=='1'){hache_sharky_outbox_release_owner($pdo,$id,$owner);break;}
            if($notAfter>0&&time()>=$notAfter){if(hache_sharky_outbox_mark_cancelled($pdo,$id,$owner,'NOT_AFTER_EXPIRED'))$stats['cancelled']++;continue;}
