...
function hache_sharky_outbox_encrypt(array $payload): array
{
    ...
    if($json===false) throw new RuntimeException('Invalid outbound payload');
    $iv=random_bytes(12);$tag='';
    $cipher=openssl_encrypt($json,'aes-256-gcm',hache_sharky_outbox_key(),OPENSSL_RAW_DATA,$iv,$tag,'sharky-outbox-v1');
...
{
    $cipher=base64_decode((string)($row['payload_ciphertext']??''),true);
    $iv=base64_decode((string)($row['payload_iv']??''),true);
    $tag=base64_decode((string)($row['payload_tag']??''),true);
    if(!is_string($cipher)||!is_string($iv)||!is_string($tag)||strlen($iv)!==12||strlen($tag)!==16) return null;
    $json=openssl_decrypt($cipher,'aes-256-gcm',hache_sharky_outbox_key(),OPENSSL_RAW_DATA,$iv,$tag,'sharky-outbox-v1');
...
function hache_sharky_outbox_enqueue(PDO $pdo,string $contact,array $payload,string $dedupeSeed): bool
{
    ...
    try{
        $sealed=hache_sharky_outbox_encrypt($payload);
        ...
        $st->execute([
            ':d'=>hash('sha256','outbox|'.$dedupeSeed),
            ':c'=>hache_sharky_orchestrator_contact_hash($contact),
            ':p'=>$sealed['ciphertext'],':iv'=>$sealed['iv'],':tag'=>$sealed['tag'],
        ]);
...
function hache_sharky_outbox_pending(PDO $pdo,int $limit=10): array
{
    $limit=max(1,min(50,$limit));
    try{return $pdo->query("SELECT id,payload_ciphertext,payload_iv,payload_tag,attempt_count FROM sharky_outbox WHERE status='PENDING' AND available_at<=NOW() ORDER BY created_at,id LIMIT ".$limit)->fetchAll(PDO::FETCH_ASSOC);}catch(Throwable $e){return [];}
}

function hache_sharky_outbox_mark_sent(PDO $pdo,string $id): void
{
    $pdo->prepare("UPDATE sharky_outbox SET status='SENT',sent_at=NOW(),last_error=NULL WHERE id=:id AND status='PENDING'")->execute([':id'=>$id]);
}

function hache_sharky_outbox_mark_failed(PDO $pdo,string $id,int $attempts,string $error='SEND_FAILED'): void
{
    $next=$attempts+1;
    $status=$next>=8?'DEAD':'PENDING';
    $delay=min(3600,30*(2**min(6,$next-1)));
    $st=$pdo->prepare('UPDATE sharky_outbox SET status=:s,attempt_count=:a,last_error=:e,available_at=DATE_ADD(NOW(),INTERVAL '.$delay.' SECOND) WHERE id=:id');
...
function hache_sharky_outbox_dispatch(PDO $pdo,callable $sender,int $limit=10): array
{
    ...
    foreach(hache_sharky_outbox_pending($pdo,$limit) as $row){
        ...
        $ok=false;
        try{$ok=$sender($payload)===true;}catch(Throwable $e){$ok=false;}
