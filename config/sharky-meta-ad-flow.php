...
function hache_sharky_meta_program_retry(): array
{
    return hache_sharky_orchestrator_decision(
        'meta_program_prompt',
        'Para orientarte mejor, elige una de estas dos opciones 👇',
        hache_sharky_meta_program_ui(false)
    );
}
...
{
    return hache_sharky_orchestrator_decision(
        'meta_regular_background_prompt',
...
function hache_sharky_meta_venue_retry(array $state): array
{
    ...
    $label=$program==='regular'?'las clases regulares':'el curso';
    return hache_sharky_orchestrator_decision(
        'meta_venue_prompt',
        'Para continuar con '.$label.', elige una sede 👇',
        hache_sharky_meta_venue_ui(false)
    );
}
...
/** @return array{morning:list<string>,evening:list<string>} */
function hache_sharky_meta_schedules(PDO $pdo,string $sede,string $program): array
{
    $column=$program==='regular'?'regular':'intensivo';
    $st=$pdo->prepare("SELECT h.hora_inicio,h.hora_fin FROM horarios h JOIN sedes s ON s.id=h.sede_id WHERE s.clave=:c AND s.activo=1 AND h.activo=1 AND h.{$column}=1 ORDER BY h.hora_inicio");
    $st->execute([':c'=>$sede]);
    $out=['morning'=>[],'evening'=>[]];
    foreach($st->fetchAll(PDO::FETCH_ASSOC) as $row){
        ...
        $bucket=$hour<12?'morning':'evening';
        $out[$bucket][]=hache_sharky_meta_schedule_label($start,$end);
    }
    $out['morning']=array_values(array_unique($out['morning']));
    $out['evening']=array_values(array_unique($out['evening']));
    return $out;
}
...
function hache_sharky_meta_venue_detail(PDO $pdo,array $state,string $sede): array
{
    $program=(string)($state['commercial_context']['program']??'');
    $business=function_exists('hache_sharky_business_values')?hache_sharky_business_values($pdo):[];
    $isMonteverde=$sede==='MONTEVERDE';
    $name=$isMonteverde?'Colegio Monteverde':'Palapas Protudec';
    $location=$isMonteverde?'Av. Bonampak, Cancún.':'Calle Alcatraces, Centro, Cancún.';
    $maps=trim((string)($business[$isMonteverde?'sharky_maps_monteverde':'sharky_maps_palapas']??''));
    if($maps==='')$maps=$isMonteverde?'https://maps.app.goo.gl/Ld75bhLforGm2Tk68':'https://maps.app.goo.gl/L7aEf9phtXtciUj78';
    $reference=$isMonteverde
        ?'La alberca está al final del estacionamiento del colegio. No necesitas entrar a la escuela; solo ingresa al estacionamiento por Av. Bonampak. Puedes utilizar el estacionamiento durante tu clase.'
    ...
    $hours=hache_sharky_meta_schedules($pdo,$sede,$program);
    $morning=$hours['morning']?implode("\n",array_map(static fn(string $h):string=>'• '.$h,$hours['morning'])):'• Sin horarios activos en este momento';
    $evening=$hours['evening']?implode("\n",array_map(static fn(string $h):string=>'• '.$h,$hours['evening'])):'• Sin horarios activos en este momento';

    $message=$name."\n\n";
    if($program==='regular'){
        $fee=hache_sharky_meta_business_int($pdo,$isMonteverde?'sharky_inscripcion_monteverde':'sharky_inscripcion_palapas',$isMonteverde?500:400);
        $message.='Inscripción: $'.number_format($fee,0,'.',',')." MXN.\n\n";
    }
    ...
    return hache_sharky_orchestrator_decision('meta_venue_detail',$message,['type'=>'buttons','buttons'=>[
        hache_sharky_meta_button($registerId,'Inscribirme'),
        hache_sharky_meta_button('meta:venue:other','Ver otra sede'),
    ]]);
}
...
function hache_sharky_meta_regular_flow_data(PDO $pdo,string $sede,int $minAge=12,int $maxAge=65,?string $today=null): ?array
{
    ...
    $st->execute([':c'=>$sede]);$site=$st->fetch(PDO::FETCH_ASSOC);if(!$site)return null;
    $sid=(string)$site['id'];

    $plans=[];
    $st=$pdo->prepare('SELECT id,nombre,sesiones_semana,precio FROM planes WHERE sede_id=:s AND activo=1 AND sesiones_semana IN (3,5) ORDER BY sesiones_semana,precio,id');
    $st->execute([':s'=>$sid]);
    foreach($st->fetchAll(PDO::FETCH_ASSOC) as $row){
        $sessions=(int)($row['sesiones_semana']??0);if(!in_array($sessions,[3,5],true))continue;
        $plans[]=['id'=>(string)$row['id'],'title'=>'Plan '.$sessions.'x · $'.number_format((float)($row['precio']??0),0,'.',',').'/mes'];
    }
    $schedules=[];
    $st=$pdo->prepare('SELECT id,hora_inicio,hora_fin FROM horarios WHERE sede_id=:s AND activo=1 AND regular=1 ORDER BY hora_inicio,id');
    $st->execute([':s'=>$sid]);
    foreach($st->fetchAll(PDO::FETCH_ASSOC) as $row){
        $schedules[]=['id'=>(string)$row['id'],'title'=>hache_sharky_meta_schedule_label((string)$row['hora_inicio'],(string)$row['hora_fin'])];
    }
    if(!$plans||!$schedules)return null;

    $tz=new DateTimeZone('America/Cancun');
    $todayObj=DateTimeImmutable::createFromFormat('!Y-m-d',$today?:date('Y-m-d'),$tz)?:new DateTimeImmutable('today',$tz);
    $minAge=max(1,$minAge);$maxAge=max($minAge,$maxAge);
    return [
        'venue_key'=>$sede,
        'venue_label'=>$sede==='MONTEVERDE'?'Colegio Monteverde':'Palapas Protudec',
        'min_birthdate'=>$todayObj->modify('-'.($maxAge+1).' years')->modify('+1 day')->format('Y-m-d'),
...
function hache_sharky_meta_regular_form(PDO $pdo,array $state,int $now,array $extraContext=[]): array
{
    ...
    $minAge=(int)($extraContext['min_age']??12);$maxAge=(int)($extraContext['max_age']??65);
    $data=hache_sharky_meta_regular_flow_data($pdo,$sede,$minAge,$maxAge,(string)($extraContext['today']??''));
    $flowId=function_exists('hache_sharky_commerce_flow_cached_id')?hache_sharky_commerce_flow_cached_id('regular_enrollment'):null;
    if(!is_array($data)||$flowId===null){
        $state=hache_sharky_orchestrator_clear_flow($state);
        return [$state,hache_sharky_orchestrator_decision(
            'regular_enrollment_unavailable',
            'No pude abrir el formulario de inscripción de forma segura. Te dejo con una persona del equipo para continuar sin hacerte repetir información.',
            [],
            ['type'=>'human_takeover']
        )];
    }
    ...
    $payload=hache_sharky_commerce_flow_payload(
        (string)($extraContext['contact']??''),
        'Completa tus datos para inscribirte a clases regulares. La sede ya queda fija en '.$data['venue_label'].'.',
        $flowId,
        'REGULAR_ENROLLMENT',
...
{
    $state=hache_sharky_orchestrator_clear_flow($state);
    return [$state,hache_sharky_orchestrator_decision('human_takeover',$message,[],['type'=>'human_takeover'])];
}
...
{
    if(!hache_sharky_meta_active($state))return null;
    if(($state['identity']['kind']??'unknown')!=='prospect')return null;
    if(($state['commercial_context']['entry_source']??'')!=='meta_ad')return null;
    ...
    if($id==='action:human'||$intent==='human')return hache_sharky_meta_human_takeover($state);
    if($intent==='student_claim')return hache_sharky_meta_human_takeover($state,'Como indicas que ya eres alumno, te dejo directamente con una persona del equipo para revisar tu expediente por este mismo chat.');
    ...
    if($step==='program'){
        if(($data['entry_bootstrap']??false)===true){
            ...
            $state=hache_sharky_orchestrator_flow($state,HACHE_SHARKY_META_FLOW,'program',$data,$now);
            return [$state,hache_sharky_meta_welcome(true)];
        }
        if($id==='meta:program:learn'){
            ...
            $state=hache_sharky_orchestrator_flow($state,HACHE_SHARKY_META_FLOW,'venue',['program'=>'intensive'],$now);
            return [$state,hache_sharky_meta_intensive_info($pdo,true)];
        }
        if($id==='meta:program:regular'){
            $state=hache_sharky_orchestrator_flow($state,HACHE_SHARKY_META_FLOW,'regular_background',[],$now);
    ...
    if($step==='regular_background'){
        ...
        if($id==='meta:regular:yes'){
            ...
            $state=hache_sharky_orchestrator_flow($state,HACHE_SHARKY_META_FLOW,'venue',['program'=>'regular'],$now);
            return [$state,hache_sharky_meta_regular_info($pdo,true)];
        }
        return [$state,hache_sharky_meta_regular_background_prompt()];
    }
    ...
    if($step==='venue'){
        ...
        $state=hache_sharky_orchestrator_flow($state,HACHE_SHARKY_META_FLOW,'venue_detail',['sede_clave'=>$sede],$now);
    ...
    if($step==='venue_detail'){
        ...
        if($id==='meta:venue:other'){
            ...
            $state=hache_sharky_orchestrator_flow($state,HACHE_SHARKY_META_FLOW,'venue_detail',['sede_clave'=>$other],$now);
            return [$state,hache_sharky_meta_venue_detail($pdo,$state,$other)];
        }
        if($id==='meta:register:intensive'&&($state['commercial_context']['program']??'')==='intensive'){
            $state=hache_sharky_orchestrator_clear_flow($state);
            $context=function_exists('hache_sharky_whatsapp_context')
                ?hache_sharky_whatsapp_context($pdo,(string)($event['from']??''),$extraContext)
                :$extraContext;
            return hache_sharky_whatsapp_registration_form_from_context($state,$context,$now,'Perfecto.');
        }
        if($id==='meta:register:regular'&&($state['commercial_context']['program']??'')==='regular'){
            $extraContext['contact']=(string)($event['from']??'');
            return hache_sharky_meta_regular_form($pdo,$state,$now,$extraContext);
        }
...
/**
 * Renderer de Meta. `images` genera una secuencia durable: fotos una sola vez y
 * después el mensaje con botones. Los retries no llevan `images`.
 */
function hache_sharky_meta_render(string $to,array $decision): array
{
    ...
    $payload=hache_sharky_whatsapp_render($to,$decision);
