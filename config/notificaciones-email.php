...
/**
 * Transporte transaccional compartido mediante la API HTTPS de Resend.
 *
 ...
 *
 * HACHE_ALERT_EMAIL_TO se usa únicamente para alertas operativas internas.
 */
function hache_email_config(): array
{
    return [
        'api_key'=>trim((string)(getenv('HACHE_RESEND_API_KEY')?:'')),
        'from'=>trim((string)(getenv('HACHE_EMAIL_FROM')?:'')),
        'from_name'=>trim((string)(getenv('HACHE_EMAIL_FROM_NAME')?:'Hache Natación')),
    ];
}
...
function hache_enviar_correo_transaccional(string $to,string $subject,string $body,?string $idempotencyKey=null): bool
{
    ...
    if($to===''||$cfg['api_key']===''||$cfg['from']===''){
        error_log('[notificaciones-email] Configuración Resend incompleta; correo omitido.');
        return false;
    }
    if(!filter_var($to,FILTER_VALIDATE_EMAIL)||!filter_var($cfg['from'],FILTER_VALIDATE_EMAIL)){
        error_log('[notificaciones-email] Dirección de correo inválida; correo omitido.');
        return false;
    }
    try{
...
function hache_resend_send(string $apiKey,string $from,string $fromName,string $to,string $subject,string $body,?string $idempotencyKey=null): bool
{
    ...
    if($response===false)throw new RuntimeException('Resend no respondió: '.$curlError);
    $decoded=json_decode((string)$response,true);
    if($httpCode<200||$httpCode>=300){
        $message=is_array($decoded)?(string)($decoded['message']??$decoded['name']??'Error desconocido'):'Respuesta HTTP '.$httpCode;
