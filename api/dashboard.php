...
try{
 ...
 $sqlSinPago="SELECT COUNT(*) FROM alumnos a INNER JOIN sedes s ON s.id=a.sede_id WHERE a.sede_id=:sede AND a.estado_administrativo='ACTIVO' AND a.plan_actual_id IS NOT NULL AND NOT EXISTS(SELECT 1 FROM mensualidades m WHERE m.alumno_id=a.id AND m.sede_id=a.sede_id AND m.estado='PAGADA' AND m.anio=CASE WHEN s.clave='PALAPAS' AND a.ciclo_pago='P15' AND DAY(CURDATE())<15 THEN YEAR(DATE_SUB(CURDATE(),INTERVAL 1 MONTH)) ELSE YEAR(CURDATE()) END AND m.mes=CASE WHEN s.clave='PALAPAS' AND a.ciclo_pago='P15' AND DAY(CURDATE())<15 THEN MONTH(DATE_SUB(CURDATE(),INTERVAL 1 MONTH)) ELSE MONTH(CURDATE()) END)";
