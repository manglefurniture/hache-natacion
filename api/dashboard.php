...
try{
 ...
 $sqlSinPago="SELECT COUNT(*) FROM alumnos a WHERE a.sede_id=:sede AND a.estado_administrativo='ACTIVO' AND a.plan_actual_id IS NOT NULL AND NOT EXISTS(SELECT 1 FROM mensualidades m WHERE m.alumno_id=a.id AND m.sede_id=a.sede_id AND m.estado='PAGADA' AND CURDATE() BETWEEN m.periodo_inicio AND m.periodo_fin)";
