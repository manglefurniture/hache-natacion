...
try {
    ...
    $instanteOperativo=new DateTimeImmutable($actualizadoEn);
    ...
    // Prospectos/conversión son CRM global y conservan el permiso ADMIN vigente.
    $prospectosConversion=(string)($me['rol']??'')==='ADMIN'
        ?dashboard_prospectos_conversion($pdo,$periodoInicio,$periodoFin)
        :['disponible'=>false,'prospectos'=>null,'conversiones'=>null,'tasa'=>null,'motivo'=>'Disponible únicamente para ADMIN.'];
    ...
    // Conserva el agregado histórico, pero total y detalle salen de la misma lectura pura.
    $mens=dashboard_mensualidades_pagadas($pdo,$sid,$hoy);
    ...
    // Conserva el conteo histórico, pero ahora expone las filas que lo componen.
    $avisosActivos=dashboard_avisos_ausencia_activos($pdo,$sid,$hoy);
    $avisos=(int)$avisosActivos['total'];
    ...
    $pendientesResumen=centro_pendientes_compuesto_resumen_activo($pdo,$sede,$includeGlobalProspects,$instanteOperativo);
    ...
    out([
        ...
        'avisos_hoy'=>$avisos,
        'avisos_hoy_detalle'=>$avisosActivos,
        ...
        'contratos'=>[
            ...
            'prospectos_conversion'=>'Cohorte global ADMIN de oportunidades/personas destinatarias creada dentro del periodo visible y desde el inicio de cobertura. Conversión exige inscripción COMPLETED; abrir Flow o iniciar pago no cuenta. SIN_SEDE permanece explícito. La conversión de una cohorte histórica puede aumentar después si la inscripción se completa más tarde.',
            ...
            'avisos_hoy'=>'Avisos de ausencia ACTIVOS de alumnos de la sede cuyo rango contiene la fecha operativa; el conteo sale exactamente de las filas expuestas.',
            'operacion_hoy'=>'Sesiones y marcas ya registradas para la fecha operativa y atribuibles a la sede por horario. La asistencia excluye sesiones CANCELADAS. Cero sesiones registradas no significa cero clases planificadas; F6 no genera sesiones ni calcula porcentaje de asistencia sin denominador estable.',
