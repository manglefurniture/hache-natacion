...
try{$periodo=financiero_validar_periodo((string)($_GET['periodo']??financiero_periodo_operativo_actual()));}catch(InvalidArgumentException $e){http_response_code(422);exit($e->getMessage());}
...
if($totalHacheMes!==null && $comisionesProaTotal!==null){
    $html.='<table class="cards"><tr><td colspan="4"><div class="k">Total Hache Natación</div><div class="v">'.money($totalHacheMes).'</div><div class="note">Participación Hache '.money($hache).' + comisiones '.esc((string)$sede['socio']).' '.money($comisionesProaTotal).'</div></td></tr></table>';
}
