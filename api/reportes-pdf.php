...
function dateLabel(string $fecha):string{
 $meses=[1=>'enero','febrero','marzo','abril','mayo','junio','julio','agosto','septiembre','octubre','noviembre','diciembre'];
 $d=new DateTimeImmutable($fecha);return (int)$d->format('j').' de '.$meses[(int)$d->format('n')].' de '.$d->format('Y');
}
...
$st->execute([':s'=>$sedeId,':mes'=>$mes,':anio'=>$anio,':si'=>$sedeId,':d'=>$desde,':h'=>$hasta]);$mensualidades=$st->fetchAll();

// 3) Intensivos agrupados por fecha real de inicio del curso.
$st=$pdo->prepare("SELECT ci.fecha_inicio,a.id,a.nombre,COALESCE(SUM(p.importe),0) importe FROM pagos p JOIN alumnos a ON a.id=p.alumno_id JOIN cursos_intensivos ci ON ci.id=p.intensivo_id WHERE p.tipo='INTENSIVO' AND p.estado='VALIDO' AND ci.sede_id=:s AND ci.fecha_inicio BETWEEN :d AND :h GROUP BY ci.fecha_inicio,a.id,a.nombre ORDER BY ci.fecha_inicio,a.nombre");
$st->execute([':s'=>$sedeId,':d'=>$desde,':h'=>$hasta]);$intensivos=$st->fetchAll();

$newRows='';foreach($nuevos as $r){$newRows.='<tr><td>'.esc((string)$r['nombre']).'</td><td class="num">'.money($r['inscripcion']).'</td><td class="num">'.money($r['mensualidad']).'</td></tr>';}
if($newRows==='')$newRows='<tr><td colspan="3" class="empty">Sin nuevas inscripciones en este periodo.</td></tr>';
$monthlyRows='';foreach($mensualidades as $r){$monthlyRows.='<tr><td>'.esc((string)$r['nombre']).'</td><td class="num">'.money($r['mensualidad']).'</td></tr>';}
if($monthlyRows==='')$monthlyRows='<tr><td colspan="2" class="empty">Sin mensualidades regulares en este periodo.</td></tr>';

$intensiveHtml='';$currentDate=null;$groupRows='';$groupTotal=0.0;$groupCount=0;
$flushIntensive=function() use (&$intensiveHtml,&$currentDate,&$groupRows,&$groupTotal,&$groupCount):void{
 if($currentDate===null)return;
 $intensiveHtml.='<div class="course"><div class="course-head"><div><span>Inicio del curso</span><b>'.esc(dateLabel($currentDate)).'</b></div><div class="course-total"><span>'.$groupCount.' alumno'.($groupCount===1?'':'s').'</span><b>'.money($groupTotal).'</b></div></div><table class="detail two"><thead><tr><th>Alumno</th><th>Pago intensivo</th></tr></thead><tbody>'.$groupRows.'</tbody></table></div>';
};
foreach($intensivos as $r){
 $fecha=(string)$r['fecha_inicio'];if($currentDate!==null&&$fecha!==$currentDate){$flushIntensive();$groupRows='';$groupTotal=0.0;$groupCount=0;}
 $currentDate=$fecha;$groupRows.='<tr><td>'.esc((string)$r['nombre']).'</td><td class="num">'.money($r['importe']).'</td></tr>';$groupTotal+=(float)$r['importe'];$groupCount++;
}
$flushIntensive();
if($intensiveHtml==='')$intensiveHtml='<div class="empty-box">Sin cursos intensivos con inicio en este periodo.</div>';

$minimoHtml=$minimo>0?('<div class="minimum"><span>Mínimo contractual '.esc($socioNombre).'</span><b>'.money($minimo).'</b><em class="'.($alcanzado?'good':'warn').'">'.($alcanzado?'Objetivo alcanzado':'Faltan '.money($faltante)).'</em></div>'):'';
