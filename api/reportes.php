...
try{
    ...
    $comisionesProaTotal=0.0;
    if((string)$sede['clave']==='MONTEVERDE'){
    ...
    out(['ok'=>true,'periodo'=>$periodo,'sede'=>['clave'=>$sede['clave'],'nombre'=>$sede['nombre'],'socio'=>$sede['socio']],'desde'=>$desde,'hasta'=>$hasta,'rango_financiero'=>$rango,'finanzas'=>$fin,'convenio_proa'=>['hache'=>$hache,'comisiones_proa_total'=>$comisionesProaTotal,'total_hache_mes'=>$totalHacheMes,'proa'=>$socio,'socio'=>$socio,'socio_nombre'=>$sede['socio'],'minimo'=>$minimo,'faltante'=>$faltante,'sobrecumplido'=>$sobrecumplido,'alcanzado'=>$alcanzado,'progreso'=>$progreso,'porcentajes'=>['mensualidades'=>(float)$sede['porcentaje_mensualidad_socio'],'intensivos'=>(float)$sede['porcentaje_intensivo_socio'],'inscripciones'=>(float)$sede['porcentaje_inscripcion_socio']]],'meses'=>array_reverse($meses),'asistencia'=>$asis]);
