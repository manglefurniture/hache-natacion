...
try{
    ...
    $comisionesProaTotal=null;$totalHacheMes=null;
    if(($me['rol']??'')==='ADMIN' && (string)$sede['clave']==='MONTEVERDE'){
        $st=$pdo->prepare("SELECT COALESCE(SUM(importe),0) FROM comisiones_proa WHERE periodo=:p");
        $st->execute([':p'=>$periodo.'-01']);
        $comisionesProaTotal=(float)$st->fetchColumn();
        $totalHacheMes=$hache+$comisionesProaTotal;
    }
    ...
    $convenio=['hache'=>$hache,'proa'=>$socio,'socio'=>$socio,'socio_nombre'=>$sede['socio'],'minimo'=>$minimo,'faltante'=>$faltante,'sobrecumplido'=>$sobrecumplido,'alcanzado'=>$alcanzado,'progreso'=>$progreso,'porcentajes'=>['mensualidades'=>(float)$sede['porcentaje_mensualidad_socio'],'intensivos'=>(float)$sede['porcentaje_intensivo_socio'],'inscripciones'=>(float)$sede['porcentaje_inscripcion_socio']];
    if($comisionesProaTotal!==null){$convenio['comisiones_proa_total']=$comisionesProaTotal;$convenio['total_hache_mes']=$totalHacheMes;}
    out(['ok'=>true,'periodo'=>$periodo,'sede'=>['clave'=>$sede['clave'],'nombre'=>$sede['nombre'],'socio'=>$sede['socio']],'desde'=>$desde,'hasta'=>$hasta,'rango_financiero'=>$rango,'finanzas'=>$fin,'convenio_proa'=>$convenio,'meses'=>array_reverse($meses),'asistencia'=>$asis]);
