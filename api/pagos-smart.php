...
$stmt=$pdo->prepare("SELECT a.id,a.sede_id,s.clave sede_clave,a.ciclo_pago,a.nombre,a.plan_actual_id,a.plan_programado_id,a.plan_programado_desde,a.horario_preferido_id,p.nombre plan_nombre,p.precio plan_precio,pp.nombre plan_programado_nombre,pp.precio plan_programado_precio FROM alumnos a JOIN sedes s ON s.id=a.sede_id LEFT JOIN planes p ON p.id=a.plan_actual_id LEFT JOIN planes pp ON pp.id=a.plan_programado_id WHERE a.id=:id LIMIT 1");$stmt->execute([':id'=>$alumnoId]);$alumno=$stmt->fetch();if(!$alumno){http_response_code(422);echo json_encode(['ok'=>false,'error'=>'Alumno no encontrado']);exit;}
if(strtoupper((string)$alumno['sede_clave'])!==$sedeClave){http_response_code(422);echo json_encode(['ok'=>false,'error'=>'El alumno no pertenece a la sede seleccionada'],JSON_UNESCAPED_UNICODE);exit;}
$sedeId=(string)$alumno['sede_id'];$ciclo=$alumno['ciclo_pago']!==null?strtoupper((string)$alumno['ciclo_pago']):null;
...
$stmt=$pdo->prepare("UPDATE alumnos SET estado_administrativo='ACTIVO',updated_at=NOW() WHERE id=:id AND estado_administrativo='PENDIENTE'");$stmt->execute([':id'=>$alumnoId]);$pdo->commit();
