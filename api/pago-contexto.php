...
try {
    ...
    $periodoInicio=new DateTimeImmutable(sprintf('%04d-%02d-01',$anio,$mes));

    $stmt=$pdo->prepare("SELECT a.id,a.nombre,a.plan_actual_id,a.plan_programado_id,a.plan_programado_desde,p.nombre plan_nombre,p.precio plan_precio,pp.nombre plan_programado_nombre,pp.precio plan_programado_precio FROM alumnos a LEFT JOIN planes p ON p.id=a.plan_actual_id LEFT JOIN planes pp ON pp.id=a.plan_programado_id WHERE a.id=:id LIMIT 1");
    ...
    $stmt=$pdo->prepare("SELECT id,estado,fecha_pago,importe_cobrado,plan_id FROM mensualidades WHERE alumno_id=:id AND mes=:mes AND anio=:anio LIMIT 1");$stmt->execute([':id'=>$alumnoId,':mes'=>$mes,':anio'=>$anio]);$mensualidadPeriodo=$stmt->fetch()?:null;
    $stmt=$pdo->prepare("SELECT mes,anio,estado,importe_a_cobrar FROM mensualidades WHERE alumno_id=:id AND estado='PENDIENTE' AND (anio<:anio1 OR (anio=:anio2 AND mes<:mes)) ORDER BY anio DESC,mes DESC LIMIT 3");$stmt->execute([':id'=>$alumnoId,':anio1'=>$anio,':anio2'=>$anio,':mes'=>$mes]);$pendientes=$stmt->fetchAll();
