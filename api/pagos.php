...
try {
     ...
     * POST
     * Registra un pago.
     * Crea automáticamente inscripción o mensualidad.
     * ==========================================================
     */
    if ($method === 'POST') {
        ...
        if (!in_array(
            $tipo,
            ['INSCRIPCION', 'MENSUALIDAD', 'INTENSIVO'],
            true
        )) {
        ...
        if ($createdBy === '') {
            ...
            echo json_encode([
                'ok' => false,
                'error' => 'No existe usuario administrativo'
            ], JSON_UNESCAPED_UNICODE);
        ...
        /*
         * Verificar usuario administrativo.
         */
        $stmt = $pdo->prepare("
            SELECT id
            FROM usuarios
        ...
        elseif ($tipo === 'MENSUALIDAD') {
            ...
            if (empty($alumno['plan_actual_id'])) {
                ...
                throw new RuntimeException(
                    'El alumno no tiene un plan activo'
                );
            }

            /*
             * Revisar si ya existe mensualidad del mismo mes.
             */
            $stmt = $pdo->prepare("
                SELECT id, estado
                FROM mensualidades
            ...
            if ($mensualidadExistente) {
                ...
                if ($mensualidadExistente['estado'] === 'PAGADA') {
                    ...
                    throw new RuntimeException(
                        'La mensualidad de este mes ya está pagada'
                    );
                }
                ...
                $mensualidadId = $pdo->query("
                    SELECT UUID()
                ")->fetchColumn();

                $importeEstandar = number_format(
                    (float)$alumno['plan_precio'],
                    2,
                    '.',
                    ''
                );
                ...
                /*
                 * La observación es obligatoria si el importe
                 * cobrado es distinto al importe estándar.
                 */
                $observacionMensualidad = $observacion;

                if (
                    (float)$importeDecimal !==
                    (float)$importeEstandar
                    &&
                    empty($observacionMensualidad)
                ) {

                    $observacionMensualidad =
                        'Importe diferente al precio estándar del plan';
                }
        ...
        elseif ($tipo === 'INTENSIVO') {
            ...
            /*
             * Buscamos un intensivo del alumno que todavía
             * pueda recibir el pago.
             */
            $stmt = $pdo->prepare("
                SELECT id
            ...
            if (!$curso) {
                ...
                throw new RuntimeException(
                    'El alumno no tiene un curso intensivo programado o en curso'
                );
            }
            ...
            /*
             * Evitar cobrar dos veces el mismo intensivo.
             */
            $stmt = $pdo->prepare("
                SELECT id
                FROM pagos
            ...
            if ($stmt->fetch()) {
                ...
                throw new RuntimeException(
                    'Este curso intensivo ya tiene un pago registrado'
                );
            }
        }
        ...
        /*
         * pagos.folio sí es AUTO_INCREMENT.
         */
