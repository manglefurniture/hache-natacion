...
try {
    ...
    if($method==='GET'){
        ...
        $includeInactive=$curso['correccion_historica_disponible']?1:0;
        $stmt=$pdo->prepare("SELECT id,hora_inicio,hora_fin,activo FROM horarios WHERE sede_id=:s AND intensivo=1 AND (activo=1 OR :hist=1) ORDER BY hora_inicio");$stmt->execute([':s'=>$sedeId,':hist'=>$includeInactive]);$horarios=$stmt->fetchAll();
