...
try {
    ...
    if($method==='POST'){
        ...
        $sincronizarFecha=hache_admin_bool($input['sincronizar_fecha_inicio']??false);
        ...
        if(mb_strlen($motivoHistorico)>700){http_response_code(422);echo json_encode(['ok'=>false,'error'=>'El motivo de la corrección no puede exceder 700 caracteres']);exit;}
        ...
        $curso['estado']=intensivo_estado_por_fechas((string)$curso['fecha_inicio'],(string)$curso['fecha_fin']);
        $inscripcionAbierta=intensivo_inscripcion_abierta((string)$curso['fecha_inicio']);
        $requiereCorreccion=!$inscripcionAbierta||$curso['estado']==='TERMINADO';
        ...
        if($historica){
            $overlap=hache_admin_historical_overlap($pdo,$alumnoId,$sedeId,$cursoId,(string)$curso['fecha_inicio'],(string)$curso['fecha_fin']);
            if($overlap){$pdo->rollBack();http_response_code(409);echo json_encode(['ok'=>false,'error'=>'No se aplicó la corrección: el alumno ya aparece en otro intensivo que se cruza con estas fechas ('.date('d/m/Y',strtotime((string)$overlap['fecha_inicio'])).'–'.date('d/m/Y',strtotime((string)$overlap['fecha_fin'])).').'],JSON_UNESCAPED_UNICODE);exit;}
            if($sincronizarFecha){
                $stmt=$pdo->prepare("SELECT ci.id FROM curso_intensivo_alumnos cia INNER JOIN cursos_intensivos ci ON ci.id=cia.curso_intensivo_id WHERE cia.alumno_id=:a AND ci.sede_id=:s AND ci.estado IN ('PROGRAMADO','EN_CURSO') LIMIT 1 FOR UPDATE");
                $stmt->execute([':a'=>$alumnoId,':s'=>$sedeId]);
                if($stmt->fetchColumn()){$pdo->rollBack();http_response_code(409);echo json_encode(['ok'=>false,'error'=>'El alumno tiene otro intensivo activo. Puedes agregar el historial, pero no sincronizar su fecha de inicio con un curso pasado mientras esa relación siga activa.'],JSON_UNESCAPED_UNICODE);exit;}
            }
        }else{
            $stmt=$pdo->prepare("SELECT ci.id FROM curso_intensivo_alumnos cia INNER JOIN cursos_intensivos ci ON ci.id=cia.curso_intensivo_id WHERE cia.alumno_id=:a AND ci.id<>:c AND ci.sede_id=:s AND ci.estado IN ('PROGRAMADO','EN_CURSO') LIMIT 1 FOR UPDATE");$stmt->execute([':a'=>$alumnoId,':c'=>$cursoId,':s'=>$sedeId]);if($stmt->fetch()){$pdo->rollBack();http_response_code(409);echo json_encode(['ok'=>false,'error'=>'El alumno ya pertenece a otro curso intensivo activo']);exit;}
        }

        $relacionObservaciones=$observaciones!==''?$observaciones:null;
        if($historica){
            $nota=hache_admin_historical_note($motivoHistorico,'Alta manual en el intensivo del '.date('d/m/Y',strtotime((string)$curso['fecha_inicio'])).'.');
            $relacionObservaciones=hache_admin_append_relation_observation($relacionObservaciones,$nota);
        }
        ...
        if($historica){
            if($sincronizarFecha){
                $stmt=$pdo->prepare("UPDATE alumnos SET fecha_inicio=:f,updated_at=NOW() WHERE id=:a AND sede_id=:s AND estado_administrativo<>'BAJA'");
                $stmt->execute([':f'=>(string)$curso['fecha_inicio'],':a'=>$alumnoId,':s'=>$sedeId]);
                $fechaSincronizada=true;
            }
            $detalle='Alumno agregado manualmente al intensivo del '.date('d/m/Y',strtotime((string)$curso['fecha_inicio'])).' para completar/corregir información histórica. Motivo: '.$motivoHistorico.'.';
            if($fechaSincronizada)$detalle.=' La fecha de inicio del alumno se ajustó a '.date('d/m/Y',strtotime((string)$curso['fecha_inicio'])).'.';
            hache_admin_history($pdo,$alumnoId,'INTENSIVO',$detalle,$createdBy,'CURSO_INTENSIVO',$cursoId);
        }else{
