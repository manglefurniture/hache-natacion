...
try {
    ...
    $stmt=$pdo->prepare("SELECT id FROM sedes WHERE clave=:c AND activo=1 LIMIT 1");
    $stmt->execute([':c'=>$sedeClave]);
    ...
    if($method==='DELETE'){
        ...
        regla_recalcular_alumno($pdo,$alumnoId);
        $pdo->commit();
    ...
    if($method==='POST'){
        ...
        $stmt=$pdo->prepare("SELECT id,estado FROM cursos_intensivos WHERE id=:id AND sede_id=:s LIMIT 1 FOR UPDATE");$stmt->execute([':id'=>$cursoId,':s'=>$sedeId]);$curso=$stmt->fetch();if(!$curso){$pdo->rollBack();http_response_code(422);echo json_encode(['ok'=>false,'error'=>'El curso intensivo no existe en la sede activa']);exit;}if($curso['estado']==='TERMINADO'){$pdo->rollBack();http_response_code(422);echo json_encode(['ok'=>false,'error'=>'No se pueden agregar alumnos a un curso terminado']);exit;}
        ...
        $stmt=$pdo->prepare("INSERT INTO curso_intensivo_alumnos(id,curso_intensivo_id,alumno_id,horario_id,observaciones,created_by) VALUES(:id,:c,:a,:h,:o,:u)");$stmt->execute([':id'=>$id,':c'=>$cursoId,':a'=>$alumnoId,':h'=>$horarioId,':o'=>$observaciones!==''?$observaciones:null,':u'=>$createdBy]);
        $stmt=$pdo->prepare("UPDATE alumnos SET plan_actual_id=NULL,horario_preferido_id=NULL,plan_programado_id=NULL,plan_programado_desde=NULL,estado_administrativo='PENDIENTE',updated_at=NOW() WHERE id=:a AND sede_id=:s AND estado_administrativo<>'BAJA'");$stmt->execute([':a'=>$alumnoId,':s'=>$sedeId]);
        regla_recalcular_alumno($pdo,$alumnoId);
        $pdo->commit();
