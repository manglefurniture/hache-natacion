...
function sede(PDO $pdo,string $clave):array{
    $st=$pdo->prepare("SELECT id,clave,nombre FROM sedes WHERE clave=:c AND activo=1 LIMIT 1");$st->execute([':c'=>$clave]);$s=$st->fetch();
    if(!$s)out(['ok'=>false,'error'=>'Sede inválida'],422);return $s;
}
...
try {
    ...
    if($method==='GET'){
        $clave=strtoupper(trim((string)($_GET['sede']??'')));
        $where='';$params=[];
        if($clave!==''&&$clave!=='TODAS'){$s=sede($pdo,$clave);$where=' WHERE a.sede_id=:sede ';$params[':sede']=$s['id'];}
        $stmt=$pdo->prepare("SELECT a.id,a.sede_id,s.clave sede_clave,s.nombre sede_nombre,a.nombre,a.fecha_nacimiento,a.whatsapp,a.correo,a.fecha_inicio,a.horario_preferido_id,a.plan_actual_id,a.plan_programado_id,a.plan_programado_desde,p.nombre AS plan_nombre,p.precio AS plan_precio,pp.nombre AS plan_programado_nombre,pp.precio AS plan_programado_precio,a.estado_administrativo,a.observaciones,a.created_at,a.updated_at FROM alumnos a INNER JOIN sedes s ON s.id=a.sede_id LEFT JOIN planes p ON p.id=a.plan_actual_id LEFT JOIN planes pp ON pp.id=a.plan_programado_id {$where} ORDER BY a.nombre ASC");
        $stmt->execute($params);$alumnos=$stmt->fetchAll();out(['ok'=>true,'total'=>count($alumnos),'alumnos'=>$alumnos]);
    }
    ...
    $stmt=$pdo->prepare("INSERT INTO alumnos(id,sede_id,nombre,fecha_nacimiento,whatsapp,correo,fecha_inicio,horario_preferido_id,plan_actual_id,estado_administrativo,observaciones) VALUES(:id,:sede,:nombre,:nac,:wa,:correo,:inicio,:horario,:plan,'PENDIENTE',:obs)");
    $stmt->execute([':id'=>$id,':sede'=>$s['id'],':nombre'=>$nombre,':nac'=>$fechaNacimiento!==''?$fechaNacimiento:null,':wa'=>$whatsapp,':correo'=>$correo!==''?$correo:null,':inicio'=>$fechaInicio,':horario'=>$horarioId,':plan'=>$planId,':obs'=>$observaciones]);
