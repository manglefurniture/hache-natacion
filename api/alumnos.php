...
try {
    ...
    if($method==='GET'){
        ...
        $s=sede($pdo,$clave);$where=' WHERE a.sede_id=:sede ';$params=[':sede'=>$s['id']];
    ...
    if($tipoIngreso==='REGULAR'){
        ...
        $stmt=$pdo->prepare("SELECT id FROM planes WHERE id=:id AND sede_id=:sede AND activo=1 LIMIT 1");$stmt->execute([':id'=>$planId,':sede'=>$s['id']]);if(!$stmt->fetch())out(['ok'=>false,'error'=>'Plan inválido para la sede seleccionada'],422);
    }else{$horarioId=null;$planId=null;}
    $pdo->beginTransaction();$id=(string)$pdo->query('SELECT UUID()')->fetchColumn();$stmt=$pdo->prepare("INSERT INTO alumnos(id,sede_id,ciclo_pago,nombre,fecha_nacimiento,whatsapp,correo,fecha_inicio,horario_preferido_id,plan_actual_id,estado_administrativo,observaciones) VALUES(:id,:sede,:ciclo,:nombre,:nac,:wa,:correo,:inicio,:horario,:plan,'PENDIENTE',:obs)");$stmt->execute([':id'=>$id,':sede'=>$s['id'],':ciclo'=>$cicloPago,':nombre'=>$nombre,':nac'=>$fechaNacimiento!==''?$fechaNacimiento:null,':wa'=>$whatsapp,':correo'=>$correo!==''?$correo:null,':inicio'=>$fechaInicio,':horario'=>$horarioId,':plan'=>$planId,':obs'=>$observaciones]);$usuario=usuario_unico($pdo,$nombre);$temporal='123456';$uid=(string)$pdo->query('SELECT UUID()')->fetchColumn();$stmt=$pdo->prepare("INSERT INTO usuarios(id,usuario,password_hash,rol,activo,debe_cambiar_password,alumno_id) VALUES(:id,:u,:p,'ALUMNO',1,1,:a)");$stmt->execute([':id'=>$uid,':u'=>$usuario,':p'=>password_hash($temporal,PASSWORD_DEFAULT),':a'=>$id]);$pdo->commit();$stmt=$pdo->prepare("SELECT a.*,s.clave sede_clave,s.nombre sede_nombre,p.nombre plan_nombre,p.precio plan_precio FROM alumnos a INNER JOIN sedes s ON s.id=a.sede_id LEFT JOIN planes p ON p.id=a.plan_actual_id WHERE a.id=:id LIMIT 1");$stmt->execute([':id'=>$id]);out(['ok'=>true,'mensaje'=>'Alumno creado correctamente. Queda pendiente hasta registrar su primer pago válido.','tipo_ingreso'=>$tipoIngreso,'alumno'=>$stmt->fetch(),'acceso_portal'=>['usuario'=>$usuario,'password_temporal'=>$temporal,'debe_cambiar_password'=>true]],201);
