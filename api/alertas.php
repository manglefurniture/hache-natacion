...
try{
 ...
 if($sinPago>0)$alertas[]=['tipo'=>'PAGO','nivel'=>'ALTA','titulo'=>"{$sinPago} alumno".($sinPago===1?'':'s')." sin mensualidad del mes",'detalle'=>'Revisar pagos pendientes antes de permitir acceso a clase.','href'=>'/alumnos.php'];
 ...
 if($aus>0)$alertas[]=['tipo'=>'AUSENCIA','nivel'=>'MEDIA','titulo'=>"{$aus} aviso".($aus===1?'':'s')." de ausencia para mañana",'detalle'=>'La asistencia los marcará como justificadas si no se presentan.','href'=>'/ausencias.php'];
 $repos=(int)$pdo->query("SELECT COUNT(*) FROM reposiciones_regulares WHERE estado='DISPONIBLE'")->fetchColumn();
 if($repos>0)$alertas[]=['tipo'=>'REPOSICION','nivel'=>'BAJA','titulo'=>"{$repos} reposición".($repos===1?'':'es')." disponible".($repos===1?'':'s'),'detalle'=>'Hay reposiciones pendientes de asignar o utilizar.','href'=>'/ausencias.php'];
 ...
 $st=$pdo->prepare("SELECT fecha_fin,COUNT(*) total FROM cursos_intensivos WHERE estado='EN_CURSO' AND fecha_fin BETWEEN :h AND :l GROUP BY fecha_fin ORDER BY fecha_fin");$st->execute([':h'=>$hoy->format('Y-m-d'),':l'=>$limite]);
 foreach($st as $r)$alertas[]=['tipo'=>'INTENSIVO','nivel'=>'MEDIA','titulo'=>'Intensivo termina el '.date('d/m',strtotime($r['fecha_fin'])),'detalle'=>'Revisar continuidad de participantes y reposiciones.','href'=>'/intensivos.php'];
 ...
 $alertas[]=['tipo'=>'PROA','nivel'=>$proa>=$minimo?'OK':'BAJA','titulo'=>$proa>=$minimo?'Mínimo PROA alcanzado ✓':'Mínimo PROA en progreso','detalle'=>$proa>=$minimo?'Las comisiones del próximo mes quedan habilitadas.':'Aporte actual: $'.number_format($proa,0).' de $'.number_format($minimo,0).'.','href'=>'/reportes.php'];
