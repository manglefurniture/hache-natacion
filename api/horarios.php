...
try{
 ...
 $in=json_decode(file_get_contents('php://input'),true)?:[];$accion=strtoupper((string)($in['accion']??''));$clave=auth_resolve_sede_clave((string)($in['sede']??'MONTEVERDE'));$sid=sedeId($pdo,$clave);
 if($accion==='CREAR'){$ini=(string)($in['hora_inicio']??'');$fin=(string)($in['hora_fin']??'');$regular=!empty($in['regular'])?1:0;$intensivo=!empty($in['intensivo'])?1:0;if(!$ini||!$fin||(!$regular&&!$intensivo))out(['ok'=>false,'error'=>'Completa horario y tipo'],422);$st=$pdo->prepare("INSERT INTO horarios(sede_id,hora_inicio,hora_fin,regular,intensivo,activo) VALUES(:s,:i,:f,:r,:n,1)");$st->execute([':s'=>$sid,':i'=>$ini,':f'=>$fin,':r'=>$regular,':n'=>$intensivo]);out(['ok'=>true]);}
 if($accion==='ACTUALIZAR'){$id=(string)($in['id']??'');$regular=!empty($in['regular'])?1:0;$intensivo=!empty($in['intensivo'])?1:0;if(!$id||(!$regular&&!$intensivo))out(['ok'=>false,'error'=>'Datos inválidos'],422);$st=$pdo->prepare("UPDATE horarios SET regular=:r,intensivo=:n,activo=:a WHERE id=:id AND sede_id=:s");$st->execute([':r'=>$regular,':n'=>$intensivo,':a'=>!empty($in['activo'])?1:0,':id'=>$id,':s'=>$sid]);out(['ok'=>true]);}
