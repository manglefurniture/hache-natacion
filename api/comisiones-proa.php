...
function monthStart(string $ym):string{if(!preg_match('/^\d{4}-(0[1-9]|1[0-2])$/',$ym))throw new InvalidArgumentException('Periodo inválido');return $ym.'-01';}
...
function proaForMonth(PDO $pdo,string $ym,array $site):array{
  ...
  $st=$pdo->prepare("SELECT COALESCE(SUM(p.importe),0) FROM pagos p INNER JOIN mensualidades m ON m.id=p.mensualidad_id WHERE p.estado='VALIDO' AND p.tipo='MENSUALIDAD' AND m.sede_id=:sede AND m.mes=:mes AND m.anio=:anio");$st->execute([':sede'=>$sedeId,':mes'=>$month,':anio'=>$year]);$mens=(float)$st->fetchColumn();
  $st=$pdo->prepare("SELECT COALESCE(SUM(p.importe),0) FROM pagos p INNER JOIN cursos_intensivos ci ON ci.id=p.intensivo_id WHERE p.estado='VALIDO' AND p.tipo='INTENSIVO' AND ci.sede_id=:sede AND DATE(p.fecha) BETWEEN :desde AND :hasta");$st->execute([':sede'=>$sedeId,':desde'=>$start,':hasta'=>$end]);$intensivos=(float)$st->fetchColumn();
  $st=$pdo->prepare("SELECT COALESCE(SUM(p.importe),0) FROM pagos p INNER JOIN inscripciones i ON i.id=p.inscripcion_id WHERE p.estado='VALIDO' AND p.tipo='INSCRIPCION' AND i.sede_id=:sede AND DATE(p.fecha) BETWEEN :desde AND :hasta");$st->execute([':sede'=>$sedeId,':desde'=>$start,':hasta'=>$end]);$inscripciones=(float)$st->fetchColumn();
  ...
  return ['mensualidades'=>$mens,'intensivos'=>$intensivos,'inscripciones'=>$inscripciones,'aporte_proa'=>$aporte];
}
try{
