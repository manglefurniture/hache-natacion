...
try {
    ...
    if(!is_array($input)){http_response_code(400);echo json_encode(['ok'=>false,'error'=>'JSON inválido'],JSON_UNESCAPED_UNICODE);exit;}
    ...
    $stmt=$pdo->prepare("SELECT id,usuario,password_hash,rol,activo,alumno_id FROM usuarios WHERE usuario=:usuario LIMIT 1");
    $stmt->execute([':usuario'=>$usuario]);$user=$stmt->fetch();
    ...
    auth_login($user);
    $pdo->prepare("UPDATE usuarios SET last_login=NOW() WHERE id=:id")->execute([':id'=>$user['id']]);
    $safe=auth_user();
    echo json_encode(['ok'=>true,'mensaje'=>'Login correcto','usuario'=>$safe,'redirect'=>$safe['rol']==='ALUMNO'?'/mi-cuenta.php':'/dashboard.php'],JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT);
