...
try {
    ...
    $stmt=$pdo->prepare("SELECT id,usuario,password_hash,rol,activo,alumno_id,debe_cambiar_password FROM usuarios WHERE usuario=:usuario LIMIT 1");$stmt->execute([':usuario'=>$usuario]);$user=$stmt->fetch();
