...
function hache_config_audit(PDO $pdo,array $me,string $key,string $before,string $after): void
{
    ...
    $st->execute([
        ':usuario_id'=>(string)$me['id'],':usuario_nombre'=>(string)($me['usuario']??''),
        ':entidad_id'=>$key,':detalle'=>$detail,
    ]);
}
...
$stmt->execute([':clave'=>$sedeClave]);$sede=$stmt->fetch();
...
if(($_SERVER['REQUEST_METHOD']??'GET')==='GET'){
    if(($me['rol']??'')==='ADMIN'){
        ...
        $cfg=array_values(array_filter($cfg,static fn(array $row):bool=>!str_starts_with((string)($row['clave']??''),'sharky_')&&!str_starts_with((string)($row['clave']??''),'f5_')));
    }else{
        ...
        $cfgStmt->execute($visibles);$cfg=$cfgStmt->fetchAll();
    }
    ...
    out([
        'ok'=>true,'sede'=>['clave'=>$sede['clave'],'nombre'=>$sede['nombre']],
        'configuracion'=>$cfg,'alertas_internas'=>hache_internal_alert_config_rows($pdo),'planes'=>$stmt->fetchAll(),
    ]);
}
...
if($accion==='CONFIG'){
    $clave=trim((string)($input['clave']??''));$valor=trim((string)($input['valor']??''));
    if(!preg_match('/^[a-z][a-z0-9_]{1,79}$/',$clave)||mb_strlen($valor)>500)out(['ok'=>false,'error'=>'Parámetro de configuración inválido'],422);
    if(str_starts_with($clave,'sharky_'))out(['ok'=>false,'error'=>'La configuración de Sharky se modifica únicamente desde Sharky Admin'],422);

    $alertDefaults=hache_internal_alert_defaults();
    if(array_key_exists($clave,$alertDefaults)){
        if(!hache_internal_alert_value_valid($clave,$valor))out(['ok'=>false,'error'=>'Valor fuera del rango permitido para esta alerta'],422);
        $check=$pdo->prepare('SELECT valor FROM configuracion WHERE clave=:clave LIMIT 1');$check->execute([':clave'=>$clave]);
        $stored=$check->fetchColumn();$before=$stored===false?(string)$alertDefaults[$clave]:trim((string)$stored);
        ...
        try{
            ...
            hache_config_audit($pdo,$me,$clave,$before,$valor);$pdo->commit();
    ...
    if($stmt->rowCount()===0){$check=$pdo->prepare('SELECT 1 FROM configuracion WHERE clave=:clave');$check->execute([':clave'=>$clave]);if(!$check->fetchColumn())out(['ok'=>false,'error'=>'Parámetro no encontrado'],404);}
    out(['ok'=>true]);
}

if($accion==='PLAN'){
    $id=trim((string)($input['id']??''));$nombre=trim((string)($input['nombre']??''));
    $sesiones=filter_var($input['sesiones_semana']??null,FILTER_VALIDATE_INT);
    $precioRaw=$input['precio']??null;$precio=is_numeric($precioRaw)?(float)$precioRaw:-1;$activo=!empty($input['activo'])?1:0;
    if($nombre===''||mb_strlen($nombre)>100||$sesiones===false||$sesiones<1||$sesiones>7||$precio<0||$precio>100000)out(['ok'=>false,'error'=>'Datos de plan inválidos'],422);
    try{
        if($id!==''){
            ...
            if($stmt->rowCount()===0){$check=$pdo->prepare('SELECT 1 FROM planes WHERE id=:id AND sede_id=:sede');$check->execute([':id'=>$id,':sede'=>$sedeId]);if(!$check->fetchColumn())out(['ok'=>false,'error'=>'Plan no encontrado en la sede activa'],404);}
        }else{
    ...
    }catch(PDOException $e){if((string)$e->getCode()==='23000')out(['ok'=>false,'error'=>'Ya existe un plan con ese nombre en esta sede'],409);throw $e;}
    out(['ok'=>true]);
}
