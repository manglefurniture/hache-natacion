...
try{
 ...
 $id=$pdo->query("SELECT UUID()")->fetchColumn();$st=$pdo->prepare("INSERT INTO cursos_intensivos(id,sede_id,fecha_inicio,fecha_fin,precio,estado,observaciones,created_by) VALUES(:id,:s,:fi,:ff,:p,'PROGRAMADO',:o,:u)");$st->execute([':id'=>$id,':s'=>$sid,':fi'=>$fi,':ff'=>$ff,':p'=>number_format((float)$precio,2,'.',''),':o'=>$observaciones!==''?$observaciones:null,':u'=>$createdBy]);
