...
{
    $st = $pdo->prepare('SELECT ENGINE FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = :table LIMIT 1');
    $st->execute([':table' => $table]);
    $engine = $st->fetchColumn();
    return ['exists' => $engine !== false,'engine' => $engine === false ? null : (string) $engine];
}

function pr_trigger_exists(PDO $pdo, string $trigger): bool
{
    $st = $pdo->prepare('SELECT 1 FROM information_schema.TRIGGERS WHERE TRIGGER_SCHEMA = DATABASE() AND TRIGGER_NAME = :trigger LIMIT 1');
...
{
    $st = $pdo->prepare('SELECT 1 FROM information_schema.STATISTICS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = :table AND COLUMN_NAME = :column AND NON_UNIQUE = 0 LIMIT 1');
...
function pr_sharky_outbox_summary(PDO $pdo): array
{
    ...
    if (!pr_table_state($pdo, 'sharky_outbox')['exists']) return ['present'=>false,'status_counts'=>[],'latest_sent_day_stored'=>null,'sent_at_semantics'=>$semantics];
    $known = ['PENDING', 'SENT', 'DEAD', 'CANCELLED'];
    $counts = array_fill_keys($known, 0);$other = 0;
    $st = $pdo->query('SELECT status, COUNT(*) AS total FROM sharky_outbox GROUP BY status');
    foreach ($st->fetchAll(PDO::FETCH_ASSOC) as $row) {
        $status = strtoupper((string) ($row['status'] ?? ''));$total = max(0, (int) ($row['total'] ?? 0));
        if (array_key_exists($status, $counts)) $counts[$status] = $total; else $other += $total;
    }
    ...
    return ['present'=>true,'status_counts'=>$counts,'latest_sent_day_stored'=>$latest===false||$latest===null||$latest===''?null:(string)$latest,'sent_at_semantics'=>$semantics];
}

try {
    $root = dirname(__DIR__);
    $config = require $root . '/config/database.php';
    if (!is_array($config)) throw new RuntimeException('database_config_invalid');
    foreach (['host', 'dbname', 'user', 'password', 'charset'] as $key) if (!array_key_exists($key, $config)) throw new RuntimeException('database_config_incomplete');
    ...
    $criticalTables = ['pagos','mensualidades','inscripciones','cursos_intensivos','curso_intensivo_alumnos','cierres_mensuales','auditoria_eventos','sharky_outbox'];
    $tables = [];foreach ($criticalTables as $table) $tables[$table] = pr_table_state($pdo, $table);
    ...
    $payload = [
        ...
        'runtime'=>['php_version'=>PHP_VERSION,'pdo_mysql'=>extension_loaded('pdo_mysql')],
        'database'=>[
    ...
    $error = ['schema_version'=>1,'collector'=>'hache-natacion-production-readiness-pilot-c','ok'=>false,'generated_at_utc'=>gmdate('c'),'error'=>'EVIDENCE_COLLECTION_FAILED'];
