...
**Última base comprobada para esta actualización:** `main`, commit `c745034fb07a21a9c683e1039c7759f71d895d9f`.  
**Estado del roadmap en esta actualización:** Fase 6 **Verificado**. Fase 7 **Desplegado**: F7.1–F7.6 están integrados y publicados; todavía faltan casos reales post-cobertura suficientes para marcar F7 **Verificado**. Fase 8 **Verificado**. Fase 9 **Verificado**: F9.0 resolvió P-09; F9.1 backend y F9.2 UI mínima están desplegados y verificados técnicamente; F9.3 quedó desplegado y verificado operativamente con evidencia real read-only.  
...
**Estado:** **Verificado.** PR #300–#309 integraron las lecturas y contratos reconciliables del dashboard; PR #311–#320 completaron autoridad/lifecycle durable de oportunidades y conversión exacta; PR #321 declaró cobertura forward-only y lectura backend reconciliable; PR #323 añadió la UI ADMIN consumiendo ese contrato sin recalcular métricas ni hacer backfill. D-18–D-22 gobiernan P-06. El 2026-09-18 se realizó la comprobación operativa dirigida en producción con sesión ADMIN y datos reales disponibles; no se reportaron discrepancias en los bloques y detalles revisados. Los ajustes visuales móviles quedan separados y no bloquean el cierre funcional. La evidencia y límites están en [F6-DASHBOARD-STATUS.md](F6-DASHBOARD-STATUS.md).
...
**Estado:** **Desplegado.** F7.1–F7.6 están integrados y publicados. La lectura ADMIN y su UI separan carga prevista/confirmada, preservan docencia compartida, sustituciones, incidencias, inactivos, fuentes, periodo y casos sin atribución sin reconstrucción ficticia. La primera evidencia operacional real sobre cobertura F7 encontró 2 profesores pero todavía cero casos post-cobertura que permitan comprobar los escenarios del criterio de terminado; por convención la fase no pasa a **Verificado**. Véase [F7-PROFESORES-STATUS.md](F7-PROFESORES-STATUS.md).
...
**Estado:** **En análisis.** F8.0 y F8.1 resolvieron P-08 mediante la matriz de cobertura y el contrato mínimo read-only de [F8-AUDITORIA-STATUS.md](F8-AUDITORIA-STATUS.md). No se hizo backfill ni se creó una segunda historia. El siguiente micro-paso es F8.2: primer backend ADMIN reconciliable.
...
**Estado:** **En implementación.** F9.0 resolvió P-09; F9.1 backend y F9.2 UI mínima quedaron desplegados y verificados técnicamente. F9.3 prepara evidencia agregada real en transacción read-only para cerrar la verificación sin fabricar casos ni exponer PII.
...
| D-24 | 2026-09-18 | La sustitución F7 es un registro explícito e independiente de `profesor_cancelaciones`: exige sesión, profesor original cubierto por una vigencia durable, sustituto distinto, motivo, origen y actor. Puede registrarse sobre una sesión `REALIZADA` dentro de cobertura como evidencia posterior; una sesión `CANCELADA` se rechaza. Una corrección anula el registro anterior y crea otro, sin reescribir historia. | F7.2 crea `profesor_sustituciones`, marcador forward-only y API ADMIN separada. Un profesor ya asignado regularmente a la misma sesión no puede etiquetarse como sustituto. No se infieren sustituciones desde cancelaciones ni se modifica Sharky. |\n| D-25 | 2026-09-18 | La primera UI de sustituciones es ADMIN-only y consume exclusivamente las autoridades F7.1/F7.2; el navegador no decide cobertura, validez ni historia. | F7.3 añade una vista separada enlazada desde Profesores para consultar periodo, registrar y anular sustituciones. Los filtros del frontend son solo de conveniencia; cualquier rechazo de negocio sigue perteneciendo al backend. |
| D-26 | 2026-09-18 | F7.4 separa carga prevista de carga realizada. La prevista se deriva de sesiones y vigencia durable de la asignación en el inicio de la clase. Una sesión `REALIZADA` con asignación regular no acredita por sí sola qué profesor la impartió: queda `sin atribución` salvo evidencia explícita. Sobre una sesión `REALIZADA`, una sustitución activa acredita al sustituto y acredita que el original no impartió; `profesor_cancelaciones` acredita indisponibilidad individual. Antes de que la sesión quede `REALIZADA`, sustituciones e incidencias siguen siendo evidencia operativa pero no carga realizada. Una sesión `CANCELADA` acredita no impartición solo para relaciones vigentes/activas, sin hacer valer sustituciones anuladas. | La lectura es ADMIN-only, por profesor y periodo, expone fuentes/coberturas y conserva docencia compartida e historial anulado. No añade migración, no escribe datos, no infiere sustituciones desde cancelaciones y no hace backfill. |
...
| 2026-09-17 | F6 P-06: cuarto micro-incremento, vínculo exacto con inscripción Sharky `COMPLETED`. | PR #316; Quality del head exitoso; Codex automático reportó un P2 sobre una acción convertida más de una vez, cubierto por `uq_sharky_prospect_conversion_action` y regresión antes del merge; producción `6d31f9a7...`; esquema, marcador y health verificados. | **Desplegado y verificado técnicamente** para este micro-alcance: conversión solo desde audit `COMPLETED`, UUID exacto, uno-a-uno por acción y reconciliación durable. El marcador de cobertura de prospectos sigue ausente; exclusión lifecycle y dashboard continúan fuera. |
| 2026-09-17 | F6 P-06: quinto micro-incremento, exclusión durable de alumno existente. | PR #318; Quality exitoso; Codex automático detectó P1 por member routing previo, corregido haciendo la reconciliación antes de routers en live/recovery y protegido por regresión; producción `d334ce5d...`. Revisión documental posterior detectó P1 adicional por lookup live degradado a `found=false`; PR #320 lo hizo fail-closed, Quality pasó y producción quedó en `c7178291...`. | **Desplegado y verificado técnicamente** para este micro-alcance: UUID exacto, `OPEN→EXCLUDED`, sin `student_id`/PII ni fallback; `CONVERTED` permanece intacto y los fallos de identidad live quedan para retry. El marcador de cobertura de prospectos sigue ausente hasta el siguiente micro-paso. |
...
| 2026-09-18 | F7.2 Profesores: autoridad explícita de sustituciones. | PR #328; Quality #1644 del head final exitoso; merge `80d0d9e917d58aa56972d680b9f6ff5a4e0a9263`. La revisión automática P2 sobre fecha operativa quedó corregida y resuelta. El deploy automático publicó exactamente el merge SHA; los PHP de API/helper/runner pasaron sintaxis y `/api/health.php` respondió HTTP 200 / `ok: true`. | **F7.2 queda desplegado y verificado técnicamente.** Sustituciones son explícitas, forward-only y no destructivas; no se infieren desde cancelaciones ni se hizo backfill. |
| 2026-09-18 | F7.3 Profesores: UI ADMIN mínima de sustituciones. | PR #330; Quality #1649 exitoso, incluida regresión UI específica; merge `7d7ad26523682bc480d1e5cbe6810b05a615a5a9`. El deploy automático publicó exactamente ese SHA; `public/profesor-sustituciones.php` pasó sintaxis, el enlace desde Profesores quedó presente y `/api/health.php` respondió HTTP 200 / `ok: true`. | **F7.3 queda desplegado y verificado técnicamente.** La vista consume F7.2 sin nueva autoridad de negocio. |
| 2026-09-18 | F7.4 Profesores: lectura backend integrada de actividad/carga. | PR #332; Quality #1657 del head final y #1658 de `main` exitosos; merge `6d9b6f0d023c71ccd239850f90bd10c62ab8f1af`; Deploy #298 publicó exactamente ese SHA. En producción, `.hache-deployed-sha` coincidió, `config/profesor-actividad.php` y `api/profesor-actividad.php` pasaron sintaxis, `/api/health.php` respondió HTTP 200 / `ok: true` y el endpoint nuevo rechazó una petición sin sesión con HTTP 401. La revisión automática de Codex no se ejecutó por límite de uso y no se invocó manualmente. | **F7.4 queda desplegado y verificado técnicamente.** Distingue carga prevista, evidencia confirmada, no impartición y sesiones realizadas sin atribución; conserva fuentes, periodo, cobertura e historia sin reconstrucción ficticia. No añadió migración, UI, backfill ni cambios a Sharky. |
| 2026-09-18 | F7.5 Profesores: UI ADMIN integrada de actividad/carga. | PR #334; Quality #1661 del PR y #1662 de `main` exitosos; merge `c0213cc25122bf05ac32c63eb8955e4b2d9b6e7f`; Deploy #300 publicó exactamente ese SHA. En producción, `.hache-deployed-sha` coincidió, `public/profesor-actividad.php` pasó sintaxis, los enlaces general y por profesor quedaron presentes, `/api/health.php` respondió HTTP 200 / `ok: true` y la vista sin sesión redirigió a autenticación. Codex automático seguía sin revisión disponible por límite de uso y no se invocó manualmente. | **F7.5 queda desplegado y verificado técnicamente.** La UI consume F7.4 sin recalcular autoridad, muestra carga prevista/confirmada, sin atribución, incidencias, sustituciones, docencia compartida, fuentes, cobertura e inactivos; sin migración, mutaciones ni backfill. |
| 2026-09-18 | F7.6 Profesores: evidencia operacional agregada y primera lectura real. | PR #336; Quality #1665 detectó un escape literal en un `require_once`, corregido antes del merge; Quality #1666 del head corregido y #1667 de `main` exitosos; merge `f976b2c5c8ef828c168aa21c7fbd020e2880b1c0`; Deploy #302 exitoso. Snapshot read-only sobre ese SHA: cobertura de asignaciones `2026-09-18 11:30:36 UTC`, sustituciones `2026-09-18 11:46:33 UTC`, 2 profesores y cero casos operativos post-cobertura; sin PII y `historia_previa_reconstruida=false`. | **F7 pasa a Desplegado, no Verificado.** El alcance funcional está en producción y los escenarios están cubiertos por regresiones, pero la cobertura real todavía no contiene los casos exigidos para comprobación operativa. Evidencia detallada en `F7-PROFESORES-STATUS.md`. |
| 2026-09-18 | F7.1 Profesores: corrección legacy de asignaciones de profesores inactivos. | PR #338; head final `beb2dde4...`; Quality #1682 del PR y #1683/#1684 de `main` exitosos; merge `6c0a298b57be4e958d67fd9723b0ced5deef0077`; Deploy #304/#305 exitosos; marcador productivo exacto y health HTTP 200 / `ok:true`. | **Corrección desplegada y verificada técnicamente.** Se desactivan asignaciones legacy inconsistentes y se colapsan baselines sintéticos abiertos sin borrar filas ni inventar historia. F7 permanece **Desplegado** hasta disponer de evidencia operativa real suficiente. |
| 2026-09-18 | F8.0/F8.1 Auditoría: diagnóstico de cobertura, resolución P-08 y contrato mínimo. | Matriz comprobada contra `auditoria_eventos`, `historial`, mutaciones administrativas, fuentes F7 y `sharky_action_audit`; sin código funcional, backfill ni permisos nuevos. Evidencia en `F8-AUDITORIA-STATUS.md`. | **En análisis**: contrato listo para un primer backend ADMIN read-only; huecos de escritura quedan separados para F8.4. |
| 2026-09-18 | F8.2/F8.3 Auditoría: lectura ADMIN unificada y UI mínima. | PR #341–#342; proyección read-only de `auditoria_eventos` + `historial`; UI consume el contrato sin reconstruir autoridad ni before/after. | **Desplegado técnicamente**; F8 continúa con huecos de escritura y evidencia final. |
| 2026-09-18 | F8.4 Auditoría: cierres forward-only de huecos de escritura. | PR #343–#349: alumno, horario rápido, profesor, retiro de intensivo, invalidación de pago, corrección de asistencia y rango financiero; #349 además evita eventos falsos en guardados sin cambios. Sin backfill ni cambios de reglas de negocio. | **Terminado y desplegado** en `26f02a9753fe7f95d0b28c6600efd21a841b930b`. La matriz fue revalidada y no quedan otros huecos administrativos importantes demostrados dentro del alcance F8. |
| 2026-09-18 | F8.5 Auditoría: regresión agregada del contrato. | PR #350; regresión agregada + `hache_auditoria_agregar()`. Codex automático reportó P2 porque la prueba inicial no ejercitaba el camino real de agregación; se corrigió con evidencias correlacionables, hilo resuelto y Quality #1714 exitoso. Merge `e4ecf4217307b4aeefed77c1f6b0719ab95151ce`; Quality #1715 de `main`; Deploy #317; marcador productivo exacto y health 200. API sin sesión devuelve 401 y UI redirige a autenticación. | **Desplegado, no Verificado**: la regresión y controles de acceso están en producción. Falta evidencia real de contenido con sesión ADMIN/casos post-cobertura. `deploy-hache` no puede leer `database.local.php`; no se ampliaron privilegios para forzar la comprobación. |
...
| 2026-09-19 | F9.2 Resumen diario: UI mínima desplegada. | PR #356; Codex detectó 1 P1 + 3 P2 y todos se corrigieron/cerraron; Quality #1735 del head y #1736 de `main` exitosos; merge `10de591cc895a1dab49dc11487ec108cf2cd6985`; Deploy #323; marcador productivo exacto, `php -l` correcto y health HTTP 200 / `ok:true`. | **Desplegado y verificado técnicamente**; F9 sigue abierto por evidencia operativa real. |
| 2026-09-19 | F9.3 Resumen diario: evidencia operativa read-only y cierre. | PR #357; Quality #1737 del head y #1738 de `main`; merge `c745034fb07a21a9c683e1039c7759f71d895d9f`; Deploy #324 y Ops Field Evidence Once #273 exitosos. El collector real completó transacción `READ ONLY`, observó actividad reciente en ambas sedes, reconcilió cobros y asistencia disponible, detectó un caso vacío natural y mantuvo la planificación histórica sin snapshot como no disponible; salida minimizada sin PII. | **F9 Verificado**: los criterios de cierre quedaron demostrados en producción sin fabricar datos ni introducir mutaciones. |
...
| F9 Resumen diario | Verificado | F1–F8, tiempo operativo, contratos F6 y D-30; F9.1/F9.2 desplegados; F9.3 evidencia operativa read-only | Fase cerrada. Mantener el contrato de lectura viva/read-only y tratar cualquier snapshot durable o ampliación de alcance como una decisión nueva. |
