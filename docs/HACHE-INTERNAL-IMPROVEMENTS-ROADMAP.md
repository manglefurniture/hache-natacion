...
**Última base comprobada para esta actualización:** `main`, commit `bed91c7a9db1dbefeeae62d7081b254651773f5e`.  
**Estado del roadmap:** Fase 1 **Implementada**; Fase 2 **Verificada**; fases 3–9 **Pendientes**.  
...
| Pendientes y alertas | F1 implementó gestión persistente para tres causas iniciales; F2 incorporó saldo de intensivo pendiente reutilizando la misma cola y quedó verificado en producción. Las alertas derivadas existentes continúan siendo fuentes separadas. | [pendientes](../api/pendientes.php), [centro de pendientes](../config/centro-pendientes.php), [alertas](../api/alertas.php), [obligaciones-alumnos](../api/obligaciones-alumnos.php). | F1 gestiona atención; F2 aporta deuda verificada; F4/F5 incorporan los tipos restantes sin duplicar la cola. |
...
**Estado:** **Verificado.** Los incrementos de PR #261 y #262 están integrados; checks, deploy y comprobación funcional ADMIN en producción quedaron completados el 2026-09-16. No queda trabajo pendiente dentro de F2 para iniciar F3; integraciones futuras de F3/F5/F6/F9 deben consumir esta definición financiera sin reabrir una contabilidad paralela.
...
| D-16 | 2026-09-16 | Cerrar F2 tras validación funcional ADMIN en producción del resumen financiero, saldo intensivo en pendientes y deep link de pago, además de checks y deploy exitosos del SHA integrado. | F2 pasa a **Verificada**; F3 puede iniciarse usando F2 como autoridad financiera cerrada. |
...
| 2026-09-16 | Verificación funcional de F2 en producción. | Sesión ADMIN: vista “Obligaciones y saldos”, Centro de pendientes y deep link hacia Pagos; marcador desplegado `bed91c7...`; regresión `CENTRO_PENDIENTES_REGRESSION_OK`. | F2 **Verificada**; se habilita el inicio de F3. |

### 12.2 Estado de las fases

| Fase | Estado | Base que se reutiliza | Próximo paso |
| --- | --- | --- | --- |
| F1 Centro de pendientes | Implementado | Alertas, obligaciones, reglas de acceso, reposiciones regulares y `pendientes_gestion` | Continuar incorporando en su momento los tipos dependientes de F4/F5 sin rehacer la cola; completar su verificación global cuando se prueben los casos restantes de F1. |
| F2 Finanzas | Verificado | Pagos, abonos, obligaciones registradas, reglas, periodos, reportes y cierres | Sin trabajo pendiente en F2 para abrir F3; las fases consumidoras deben reutilizar esta definición financiera. |
