...
**Estado:** En implementación. F7.1 está desplegado y verificado técnicamente. F7.2 implementa el registro explícito forward-only de sustituciones con conservación de anulaciones; no infiere sustituciones desde cancelaciones ni reconstruye historia anterior. Incidencias, carga realizada e historial/UI integrada continúan pendientes en micro-pasos posteriores.
...
| 2026-09-18 | F7.2 Profesores: autoridad explícita de sustituciones. | Micro-paso en implementación: tabla/marker forward-only, API ADMIN separada, anulación no destructiva y regresión MariaDB. No toca `profesor_cancelaciones`, Sharky ni sesiones/asistencia. | Pendiente de Quality, merge, deploy y verificación antes de marcar F7.2 como cerrado. |
...
| F7 Profesores | En implementación | Profesores, horarios, cancelaciones, D-23/D-24 y F7.1 forward-only | F7.2 en implementación: sustituciones explícitas. No iniciar el siguiente hueco hasta cerrar este micro-paso. |
