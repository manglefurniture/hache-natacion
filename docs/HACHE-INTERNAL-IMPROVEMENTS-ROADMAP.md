...
| D-26 | 2026-09-18 | F7.4 separa carga prevista de carga realizada. La prevista se deriva de sesiones y vigencia durable de la asignación en el inicio de la clase. Una sesión `REALIZADA` con asignación regular no acredita por sí sola qué profesor la impartió: queda `sin atribución` salvo evidencia explícita. Una sustitución activa más sesión `REALIZADA` acredita al sustituto; la sustitución acredita que el original no impartió, `profesor_cancelaciones` acredita indisponibilidad individual y una sesión `CANCELADA` acredita no impartición. | La lectura es ADMIN-only, por profesor y periodo, expone fuentes/coberturas y conserva docencia compartida e historial anulado. No añade migración, no escribe datos, no infiere sustituciones desde cancelaciones y no hace backfill. |
