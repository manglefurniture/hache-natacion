...
| 2026-09-17 | F6 P-06: quinto micro-incremento, exclusión durable de alumno existente. | PR #318; Quality exitoso; Codex automático detectó P1 por member routing previo, corregido haciendo la reconciliación antes de routers en live/recovery y protegido por regresión; producción `d334ce5d...`; marcador, sintaxis, hooks y health verificados. | **Desplegado y verificado técnicamente** para este micro-alcance: UUID exacto, `OPEN→EXCLUDED`, sin `student_id`/PII ni fallback; `CONVERTED` permanece intacto. El marcador de cobertura de prospectos sigue ausente hasta el siguiente micro-paso. |
...
| F6 Dashboard | En implementación | Dashboard, tiempo operativo, F1/F2/F5, P-06 resuelta por D-18/D-19/D-20/D-21, lecturas puras, productor durable, sede estructurada, vínculo `COMPLETED` y exclusión de alumno existente desplegados | Declarar cobertura forward-only y añadir primero la lectura backend reconciliable de prospectos/conversiones por cohorte, sede y fuente; UI en micro-paso posterior si el contrato pasa pruebas, sin backfill. |
