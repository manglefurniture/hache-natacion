...
function respuesta_reintentable(array $item): bool
{
    return $item['reply_to_id']!==null
        ...
        &&$item['reply_root_estado']==='APROBADO'
        &&$item['reply_to_estado']==='APROBADO'
        &&$item['reply_target_aviso_estado']==='ACTIVA'
        &&!in_array((string)$item['notificacion_estado'],['ENVIADA','NO_APLICA'],true)
        &&(int)$item['notificacion_intentos']<3;
}
function correo_reintentable_detalle(PDO $pdo,string $id): bool
{
    $st=$pdo->prepare("SELECT c.estado,r.reply_to_id,r.notificacion_estado,r.notificacion_intentos,root.estado reply_root_estado,target.estado reply_to_estado,target_s.estado reply_target_aviso_estado,s.estado aviso_estado,s.confirmacion_estado,s.confirmacion_intentos,s.confirm_expires_at FROM historia_comentarios c LEFT JOIN historia_respuestas r ON r.comentario_id=c.id LEFT JOIN historia_comentarios root ON root.id=r.parent_id LEFT JOIN historia_comentarios target ON target.id=r.reply_to_id LEFT JOIN historia_comentario_suscripciones target_s ON target_s.comentario_id=r.reply_to_id LEFT JOIN historia_comentario_suscripciones s ON s.comentario_id=c.id WHERE c.id=:id LIMIT 1");
...
try{
    ...
    if($method==='GET'){
        ...
        foreach($items as &$item){
            ...
            unset($item['confirm_expires_at'],$item['reply_root_estado'],$item['reply_to_estado'],$item['reply_target_aviso_estado']);
        }unset($item);
