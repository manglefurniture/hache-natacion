...
try{
    if(strtoupper((string)($_SERVER['REQUEST_METHOD']??'GET'))!=='GET'){
        ...
        $message='Este enlace solo puede abrirse desde el correo que recibiste.';
    }else{
        $accion=strtolower(trim((string)($_GET['accion']??'')));
        $token=trim((string)($_GET['token']??''));
        if(!preg_match('/^[A-Za-z0-9_-]{43}$/',$token)){
            http_response_code(400);
            $message='El enlace no es válido o está incompleto.';
            ...
            $st->execute([':hash'=>hash('sha256',$token)]);
            $row=$st->fetch();
            if(!$row){
                http_response_code(404);
                $message='Este enlace de confirmación ya no es válido.';
            }else{
                $storyUrl='/historias/'.rawurlencode((string)$row['historia_slug']).'.php#comentario-'.rawurlencode((string)$row['comentario_id']);
                if($row['estado']==='ACTIVA'){
                    $ok=true;
                    $message='Los avisos de respuestas ya estaban activados para este comentario.';
                }elseif($row['estado']==='CANCELADA'){
                    $message='Los avisos de este comentario fueron cancelados y este enlace ya no puede reactivarlos.';
                }elseif(strtotime((string)$row['confirm_expires_at'])<time()){
                    http_response_code(410);
                    $message='El enlace de confirmación venció. No se activaron avisos.';
                }else{
                    $up=$pdo->prepare("UPDATE historia_comentario_suscripciones SET estado='ACTIVA',confirmado_at=NOW(),updated_at=NOW() WHERE comentario_id=:id AND estado='PENDIENTE'");
                    $up->execute([':id'=>$row['comentario_id']]);
                    $ok=$up->rowCount()===1;
            ...
            $comentario=trim((string)($_GET['comentario']??''));
            if(!preg_match('/^[a-f0-9-]{36}$/i',$comentario)){
                http_response_code(400);
                $message='El enlace para cancelar avisos no es válido.';
            }else{
                ...
                $st->execute([':id'=>$comentario]);
                $row=$st->fetch();
                if(!$row){
                    http_response_code(404);
                    $message='No encontramos avisos activos para este comentario.';
                }else{
                    $storyUrl='/historias/'.rawurlencode((string)$row['historia_slug']).'.php#comentario-'.rawurlencode($comentario);
                    $secret=historias_notificacion_secreto($config);
                    $expected=historias_cancel_token($comentario,(string)$row['email'],$secret);
                    if(!hash_equals($expected,$token)){
                        http_response_code(403);
                        $message='El enlace para cancelar avisos no es válido.';
                    }elseif($row['estado']==='CANCELADA'){
                        $ok=true;
                        $message='Los avisos de este comentario ya estaban cancelados.';
                    }else{
                        ...
                        $up->execute([':id'=>$comentario]);
                        $ok=true;
                        $message='Listo. Ya no recibirás avisos de respuestas a este comentario.';
                    }
                }
            }
        }else{
    ...
    error_log('[historias/notificaciones] '.$e->getMessage());
    http_response_code(500);
    $message='No pudimos procesar el enlace en este momento. Intenta nuevamente más tarde.';
}
?>
<!doctype html>
<html lang="es-MX">
...
<body>
  <main class="card">
    ...
    <h1><?= $ok?'Todo listo.':'Revisa este enlace.' ?></h1>
