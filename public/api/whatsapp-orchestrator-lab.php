...
{
    $version = sharky_lab_secret('WHATSAPP_GRAPH_VERSION');
    return preg_match('/^v\d+\.\d+$/', $version) === 1 ? $version : 'v26.0';
}

function sharky_lab_send(array $payload): bool
{
    $token = sharky_lab_secret('WHATSAPP_ACCESS_TOKEN');
    $phoneId = sharky_lab_secret('WHATSAPP_PHONE_NUMBER_ID');
...
{
    $history = [];
    $ref = $state['referral']['latest'] ?? null;
    if (is_array($ref) && !empty($ref['headline'])) {
        $history[] = ['role'=>'system', 'content'=>'Origen de campaña: '.mb_substr((string)$ref['headline'], 0, 180)];
    }
    $history[] = ['role'=>'system', 'content'=>$instruction];
    $payload = json_encode(['message'=>$text, 'history'=>$history, 'channel'=>'whatsapp'], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    ...
    return is_array($data) && ($data['ok'] ?? false) === true ? trim((string) ($data['answer'] ?? '')) : '';
}
...
if ($method === 'GET') {
    ...
    $expected = sharky_lab_secret('WHATSAPP_VERIFY_TOKEN');
...
$secret = sharky_lab_secret('META_APP_SECRET');
...
if (!is_array($payload)) sharky_lab_json(400, ['ok'=>false, 'error'=>'Invalid JSON']);

$events = array_merge(hache_sharky_whatsapp_extract($payload), hache_sharky_draft_extract_audio_events($payload));
usort($events, static fn(array $a, array $b): int => (int)($a['timestamp_ms'] ?? 0) <=> (int)($b['timestamp_ms'] ?? 0));
...
if (!$pdo instanceof PDO) {
    error_log('[sharky-orchestrator] lab database unavailable; event ignored safely');
    exit;
}
$business = hache_sharky_business_values($pdo);
$minAge = hache_sharky_config_int($business, 'sharky_edad_minima', 12, 1, 99);
$escalationThreshold = hache_sharky_config_int($business, 'sharky_escalado_intentos', 2, 1, 5);
$configured = sharky_lab_secret('WHATSAPP_PHONE_NUMBER_ID');
$secretResolver = static fn(string $name): string => sharky_lab_secret($name);

foreach ($echoes as $echo) {
    if ($configured !== '' && ($echo['phone_number_id'] ?? '') !== '' && !hash_equals($configured, (string)$echo['phone_number_id'])) continue;
    hache_sharky_takeover_mark((string)$echo['to'], 'manual', 'Respuesta manual detectada por WhatsApp coexistence.');
}
...
foreach ($events as $event) {
    ...
    if ($contact === '' || hache_sharky_takeover_active($contact)) continue;

    if (($event['type'] ?? '') === 'audio') {
        $text = hache_sharky_draft_transcribe_audio($event, $business, $secretResolver);
        if ($text === '') {
            sharky_lab_send(hache_sharky_whatsapp_text_payload($contact, 'No pude procesar esa nota de voz. Escríbeme el mensaje y seguimos por aquí.'));
            continue;
        }
        $event['type'] = 'text';
        $event['text'] = $text;
    ...
    if ($text !== '' && hache_sharky_draft_requires_handoff($text)) {
        ...
        sharky_lab_send(hache_sharky_whatsapp_text_payload($contact, 'Te dejo con el equipo de Hache Natación. Una persona continuará contigo por este mismo chat.'));
        continue;
    }

    $result = hache_sharky_whatsapp_enqueue($pdo, $event, 'sharky_lab_answer', [
        'verification_base_url'=>'https://hnatacion.com/sharky-verificar.php',
        'min_age'=>$minAge,
    ]);
    if ($result['skip'] ?? false) continue;

    $decision = is_array($result['decision'] ?? null) ? $result['decision'] : [];
    $action = is_array($decision['action'] ?? null) ? $decision['action'] : null;
    if (is_array($action) && ($action['type'] ?? '') === 'human_takeover') {
        hache_sharky_takeover_mark($contact, 'requested_human', 'Sharky 2.0 controlled handoff');
    }
    ...
    if (is_array($out)) sharky_lab_send($out);
}
exit;
