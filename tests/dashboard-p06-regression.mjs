...
const liveWebhook=fs.readFileSync(new URL('../public/api/whatsapp-orchestrator-lab.php',import.meta.url),'utf8');
...
assert.match(helper,/FROM sharky_prospect_opportunities/);
...
echo json_encode([
 ...
 dashboard_prospectos_conversion($pdo,'2026-09-01','2026-09-30')
]);`;
...
assert.equal(prospectConversion.prospectos,2);
assert.equal(prospectConversion.conversiones,1);
assert.equal(prospectConversion.tasa,50);
assert.equal(prospectConversion.parcial,true);
assert.equal(prospectConversion.por_sede.MONTEVERDE.prospectos,1);
...
assert.equal(prospectConversion.por_sede.PALAPAS.prospectos,0);
...
assert.match(page,/Prospectos/);
assert.match(page,/Conversión/);
...
assert.match(migrationRunner,/sharky_prospect_opportunities/);
...
assert.match(actionRecovery,/\['register_intensive','register_regular'\]/);
assert.match(actionRecovery,/hache_sharky_prospect_opportunity_convert/);
assert.match(liveWebhook,/hache_sharky_prospect_opportunity_ensure/);

console.log('dashboard P-06 lifecycle, attendance and conversion regression: OK');
