...
$birthState=$x['state'];
eq(hache_sharky_orchestrator_parse_birthdate('07 febrero 1984',$today),'1984-02-07','Spanish month name birthdate is accepted');
eq(hache_sharky_orchestrator_parse_birthdate('7 de febrero de 1984',$today),'1984-02-07','natural Spanish birthdate with de is accepted');
eq(hache_sharky_orchestrator_parse_birthdate('07021984',$today),'1984-02-07','compact DDMMYYYY birthdate is accepted');
eq(hache_sharky_orchestrator_parse_birthdate('20011984',$today),'1984-01-20','compact DDMMYYYY that begins like a plausible year must stay day-first');
eq(hache_sharky_orchestrator_parse_birthdate('19840207',$today),'1984-02-07','compact YYYYMMDD falls back only when DDMMYYYY is invalid');
eq(hache_sharky_orchestrator_parse_birthdate('1984-02-07',$today),'1984-02-07','ISO birthdate remains accepted');
eq(hache_sharky_orchestrator_parse_birthdate('31021984',$today),null,'impossible compact birthdate remains invalid');
$natural=hache_sharky_orchestrate($birthState,['id'=>'reg.birth.natural','from'=>'529900000003','text'=>'07 febrero 1984'],['now'=>$now+7,'today'=>$today,'intensive_options'=>$options,'min_age'=>12]);
eq($natural['decision']['kind'],'registration_confirm','screenshot-style month-name date reaches confirmation');
eq($natural['state']['flow']['data']['birthdate'],'1984-02-07','month-name date is normalized before persistence');
ok(str_contains($natural['decision']['message'],'07/02/1984'),'confirmation echoes normalized birthdate for user verification');
$compact=hache_sharky_orchestrate($birthState,['id'=>'reg.birth.compact','from'=>'529900000003','text'=>'07021984'],['now'=>$now+7,'today'=>$today,'intensive_options'=>$options,'min_age'=>12]);
eq($compact['decision']['kind'],'registration_confirm','screenshot-style compact date reaches confirmation');
eq($compact['state']['flow']['data']['birthdate'],'1984-02-07','compact date is normalized before persistence');
$x=hache_sharky_orchestrate($birthState,['id'=>'reg.birth','from'=>'529900000003','text'=>'01/01/2000'],['now'=>$now+7,'today'=>$today,'intensive_options'=>$options,'min_age'=>12]);
