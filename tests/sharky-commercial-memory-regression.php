...
$numericCatalog=$catalog;
$numericCatalog['courses'][]=['id'=>'c21','sede_clave'=>'PALAPAS','fecha_inicio'=>'2026-09-21','precio'=>1350,'schedules'=>[['id'=>'h20','label'=>'20:00–21:00']]];
[$numericDate]=memory_turn($ignacia,'21/09/2026',$numericCatalog);
memory_ok(($numericDate['commercial_context']['course_id']??null)==='c21'&&($numericDate['commercial_context']['fecha_inicio']??null)==='2026-09-21','Numeric date must replace an already confirmed course when available.');
[$numericUnavailable]=memory_turn($ignacia,'21/09/2026',$catalog);
memory_ok(empty($numericUnavailable['commercial_context']['course_id'])&&hache_sharky_commercial_next($numericUnavailable)['slot']==='course','Unavailable numeric date must clear the previous confirmed course.');
[$monthChange]=memory_turn($ignacia,'De octubre',$catalog);
memory_ok(empty($monthChange['commercial_context']['course_id'])&&($monthChange['commercial_context']['date_preference']['month']??null)==='octubre'&&hache_sharky_commercial_next($monthChange)['slot']==='course','A new month preference must clear the previously confirmed course.');
$longDecision=hache_sharky_orchestrator_decision('commercial_progress',str_repeat('X',1024),['type'=>'buttons','buttons'=>[hache_sharky_orchestrator_button('action:human','Inscribirme')]]);
$longPayload=hache_sharky_whatsapp_render('529981112233',$longDecision);
$limitedPayload=hache_sharky_lab_present_once($longPayload,memory_new(),'Hola');
