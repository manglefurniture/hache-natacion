...
sharky_activation_expect(str_contains($inbox,"SHARKY_ORCHESTRATOR_LAB_ENABLED')!=='1'")&&str_contains($inbox,'"disabled":true'),'Inbox worker must stop processing automatically when the lab flag is off.');
sharky_activation_expect(str_contains($outbox,"SHARKY_ORCHESTRATOR_LAB_ENABLED')!=='1'")&&str_contains($outbox,'"disabled":true'),'Outbox worker must stop sending automatically when the lab flag is off.');
