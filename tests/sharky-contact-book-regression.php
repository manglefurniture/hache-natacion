...
contact_book_expect(str_contains($maintenance,"--status")&&str_contains($maintenance,"--sync-once")&&str_contains($maintenance,"--set-refresh-token")&&str_contains($maintenance,"--exchange-auth-code"),'Google Contacts recovery must support status, one-shot sync, refresh-token replacement and authorization-code exchange.');
...
contact_book_expect(str_contains($maintenance,"https://oauth2.googleapis.com/token")&&str_contains($maintenance,"https://developers.google.com/oauthplayground")&&str_contains($maintenance,"grant_type'=>'authorization_code"),'Authorization-code recovery must use Google OAuth token exchange with the registered Playground redirect URI.');
...
contact_book_expect(str_contains($oauthApi,"HACHE_GOOGLE_CONTACTS_OAUTH_STAGE_DIR='/var/lib/hache-natacion/oauth-stage'")&&str_contains($oauthApi,'HACHE_GOOGLE_CONTACTS_OAUTH_STAGE_FILE')&&str_contains($oauthApi,'chmod($path,0600)'),'OAuth renewal must stage credentials in the shared private directory outside PHP-FPM PrivateTmp.');
...
contact_book_expect(str_contains($maintenance,"'/var/lib/hache-natacion/oauth-stage/google-contacts-oauth-stage.json'")&&str_contains($maintenance,'hache_google_contacts_promote_staged()')&&str_contains($maintenance,'time()-900')&&str_contains($maintenance,'hache_google_contacts_store_credentials($clientSecret,$refreshToken)')&&str_contains($maintenance,'@unlink($path)'),'Privileged maintenance must promote only fresh shared staged credentials into the authoritative environment and delete staging.');
...
contact_book_expect(str_contains($wrapper,'GOOGLE_CONTACTS_OAUTH_STAGE_DIR="${GOOGLE_CONTACTS_OAUTH_ROOT}/oauth-stage"')&&str_contains($wrapper,'install -d -o www-data -g www-data -m 0700 "$GOOGLE_CONTACTS_OAUTH_STAGE_DIR"')&&str_contains($wrapper,'ensure_google_contacts_oauth_stage_dir'),'Privileged wrapper must provision the shared OAuth staging directory for www-data with mode 0700.');
