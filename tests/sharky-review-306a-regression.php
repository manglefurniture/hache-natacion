...
sharky_review_306a_expect(str_contains($recovery,"$row['result_decrypt_failed']=!is_array($decoded)")||str_contains($recovery,"\$row['result_decrypt_failed']=!is_array(\$decoded)"),'Encrypted result decrypt failure must be explicit.');
...
sharky_review_306a_expect(str_contains($executor,"($existing['result_decrypt_failed']??false)===true")||str_contains($executor,"(\$existing['result_decrypt_failed']??false)===true"),'Executor must fail closed on unreadable completed results.');
