...
portal_expect(str_contains($passwordApi,'hache_portal_access_revoke_user'),'Crear la contraseña debe revocar enlaces pendientes.');
...
portal_expect(str_contains($template,"HACHE_SHARKY_TEMPLATE_ENROLLMENT_CONFIRMED = 'hache_registro_recibido_mx'"),'Debe usarse la nueva plantilla aprobable.');
...
portal_expect(str_contains($template,'hache_portal_access_issue($pdo,$studentId)'),'Cada registro debe emitir su acceso seguro.');
portal_expect(str_contains($registration,'La confirmación y el acceso al portal se enviarán al número que registraste.'),'El registro web debe dirigir al WhatsApp.');
