...
backfill_send_once_ok(str_contains($sender,"(int)(\$state['updated_at'] ?? 0) !== \$userTurnAt")&&str_contains($sender,'hache_sharky_followup_commercial_ready')&&str_contains($sender,'hache_sharky_followup_context_matches'),'Sender must fail closed on stale state or changed commercial context.');
